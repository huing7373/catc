...
sequenceDiagram
    ...
    Service->>MySQL: 提交事务
    Note over Service,WSGateway: 事务提交后（post-commit）触发主动关闭 + 广播<br/>**严格顺序（r13 锁定）**：close 4007 + unregister leaver Session → broadcast `member.left` → HTTP 200<br/>（先 close + unregister 后 broadcast：BroadcastToRoom primitive 不带 `excludeUserID` 参数 → fanout 给 ListSessionsByRoomID 全部 session；先 close 后 broadcast 保证 leaver Session 已不在列表中，fanout 物理上跳过 leaver，无需扩展 primitive。与 V1接口设计.md §10.5 步骤 7/8/9 zip 对齐）<br/>**注（r10 锁定）**：close 4007 是 server 端 best-effort cleanup（HTTP 与 WS 走两条独立连接，无 ordering / delivery 保证；4007 可能比 HTTP 200 晚到 / leaver WS 已断 / 4007 丢包均合法）；**leave 完成的 authoritative signal 是 HTTP 200 响应**，client 必须以 HTTP 200 为推进 RoomView 退出 / 清房间状态的唯一触发点
    Service->>WSGateway: CloseLeaverConnection(userId) + Unregister leaver Session
    WSGateway-->>Client: WS Close 4007 "left room via HTTP" (best-effort, 可能不到达)
    Service->>WSGateway: BroadcastToRoom(roomId, member.left)
    WSGateway->>Others: WS 推送 member.left {userId}（fanout 列表已不含 leaver）
    Service-->>API: 返回 left = true
    API-->>Client: HTTP 200 退出成功（authoritative signal）
```
...
- 退出房间必须显式调用 HTTP `POST /api/v1/rooms/{roomId}/leave` 接口（无被动 leave 路径；§10.5 步骤 7 的 close 4007 是 server 端 best-effort cleanup —— 不是独立"离开"路径，由 HTTP leave 事务触发，且 client 推进 leave UX 必须以 HTTP 200 为唯一权威信号，不等 4007）
