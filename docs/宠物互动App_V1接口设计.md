...
- 任何字段名 / 字段类型 / `slot` 枚举（`{1,2,3,4,5,6,7,99}`）/ `rarity` 枚举（`{1,2,3,4}`）/ `instances[].status` 枚举（`{1,2}`，inventory 仅返回 in_bag + equipped）/ `groups` 按 `cosmetic_item_id` 聚合维度 / `count` = `instances` 数组长度（含 equipped）语义 / catalog 排序 `rarity ASC + slot ASC + id ASC`（全序确定，`id ASC` 为决定性 tie-breaker，client 可依赖整个列表顺序）/ **inventory 两级确定性全序排序**（`groups[]` = `rarity ASC + slot ASC + cosmeticItemId ASC` 与 catalog 同风格、`instances[]` = `userCosmeticItemId ASC`，两级 tie-breaker 均到唯一键级别保证全序唯一，client 可依赖两级列表顺序，fix-review 23-1 r3 [P2]）/ 空背包 `{groups: []}` 契约 / `iconUrl` / `assetUrl` **非空字符串**契约（enabled cosmetic 正常行 / catalog 全量返回 / inventory **态 A enabled + 态 B disabled-but-exists** 组 `1 ≤ length ≤ 255` 非空，与 §7.2 `reward.iconUrl` / `reward.assetUrl` 非空冻结对齐；空字符串 `""` **仅**限 §8.2 **态 C missing-no-row**（admin 物理删 row、数据不可得）降级占位路径，**不**适用于态 A·B 行 —— 此契约 fix-review r2 [P2] 收紧、r3 [P2] 把非空范围明确扩展到态 B，消除"catalog 空 URL vs chest-open 非空冻结"跨接口矛盾） / **inventory config 三态完整矩阵**（态 A enabled → row 真实 metadata；态 B disabled-but-exists `is_enabled = 0` 仍存 row → 仍返回 + row 真实 metadata + 不 log；态 C missing-no-row → 仍返回 + 降级占位 + log error；Story 23.4 关联 `cosmetic_items` **禁止**加 `is_enabled = 1` 过滤，已拥有不得静默丢失，fix-review 23-1 r3 [P2]） / 不分页声明 / 错误码（1001 / 1005 / 1009）触发条件 的修改都必须（**冻结边界说明**：`cosmetic_items` 行的 admin 增删（满足 AR18 数量约束前提下）/ `user_cosmetic_items` 行的正常业务增减（开箱 / 合成产生）/ §7.2 `reward.userCosmeticItemId` 节点 7 → 节点 8 真实化（已在 §1 Story 20.1 冻结边界说明锚定，字段类型 / 名 / 必填性不变，仅服务端语义升级）**不**视为契约变更；1001 / 1005 / 1009 等"触发条件"冻结在**抽象层** —— "走 §3 错误码表对应映射"这一不变量，**具体限频阈值**（如 60/min）由 Story 4.5 默认值 + 配置层管理，调整阈值**不**视为本接口契约变更；删除 auth / rate_limit 中间件 / 改聚合维度 / 改 status 过滤范围 / 把 `count` 改成"仅 in_bag 计数" / 把空背包改成 `null` 或省略 groups 字段 / 把 catalog 排序改掉 / 改任一级 inventory 排序键 / 把 config 三态退化为只处理部分态（如对 disabled / 缺失 skip）/ 引入分页 才视为契约变更）：
...
1. **认证**：auth 中间件校验 Bearer token（必走；**注**：rate_limit 中间件**不**在本接口的 middleware 链上 —— 见步骤 4 + 本节末「关键约束」「rate_limit 位置 r10 调整 + r11 修订」段；这是本接口对"统一 rate_limit middleware"基线的契约级例外，由 r10 review 锁定，r11 review 收紧"免配额"边界）
2. **参数校验**：`idempotencyKey` 必填 + length 1-128 + 字符集 `[A-Za-z0-9_:-]` —— 不满足 → 1002
3. **committed success 幂等命中预检**（**仅检查 committed success 行**；handler 入口，**不开** MySQL 事务 —— 这一步是"轻量只读 SELECT + 短路返回"；**r11 review 锁定**：仅命中 `status = 'success'` 时短路 + 免 rate_limit；**不再**做 "pending 命中预检"（autocommit SELECT 在 MVCC 下看不到首事务尚未 commit 的 pending 行，r10 设计的 "pending 命中 → 免配额 → 返回 1008" 路径**不可实现**，详见本节末「关键约束」「MVCC 下 pending 不可见 r11 决策」段）：
   ...
   - **命中 `status = 'success'`**（首次开箱已成功，`response_json` 已写入并 commit）→ 跳过步骤 4（**不**计入 rate_limit 配额）+ 跳过步骤 5（**不**进业务事务）→ **反序列化 `response_json` + 补算 `data.nextChest.status = (unlock_at > now) ? 1 : 2` + `data.nextChest.remainingSeconds = max(0, ceil((nextChest.unlockAt - now) / 1s))`（两字段**同源同时刻**计算，用同一个 `now` 快照；r11 review 锁定：与 §7.1 GET /chest/current 完全对齐，与步骤 5b cached 路径同处理） + 填入当前请求的 `requestId`**（`response_json` 缓存**不**包含 `nextChest.status` / `nextChest.remainingSeconds` / `requestId`，由 server 在响应序列化时实时填入；详见步骤 5j 序列化说明）→ 返回 `200`
   - **未命中**（行不存在）→ 继续步骤 4（限频检查）。**注**：**MVCC 决定**本 autocommit SELECT 在两类 case 下都会"未命中"：(a) 行真不存在（首次到达 / 首次事务已 rollback 后到达）/ (b) 首事务已 INSERT pending 但**尚未 commit**（pending 行对 autocommit 的 read view 不可见 —— InnoDB 默认 REPEATABLE READ 不读未提交数据）。两类未命中在步骤 4 / 5 都按"真新请求 / 同 key 并发"路径处理 —— pending 不可见的情况由步骤 5 的 InnoDB unique-key X-lock 阻塞排队兜底（详见步骤 5a「InnoDB 锁语义」+ 本节末「关键约束」「MVCC 下 pending 不可见 r11 决策」段）
4. **限频检查**（**对所有未命中 committed success 的请求做**，包括 (a) 真新请求 + (b) **pending 阶段同 key 重试** —— **r11 review 锁定**：MVCC 决定步骤 3 看不到首事务的 pending 行，pending 阶段同 key 重试在步骤 3 表现为"未命中"，故**会**走步骤 4 消耗 rate_limit 配额；这是 r11 对 r10 "pending 命中免配额" 承诺的修订，详见本节末「关键约束」段）；handler 内层，**不**在 middleware 链上：按 `user_id` 维度，每用户每分钟限 60 次（与 §4.5 钦定的默认 rate_limit policy 一致）—— 超限 → 返回 **1005 操作过于频繁**；未超 → 继续步骤 5
5. **MySQL 事务开始**（`txManager.WithTx(ctx, fn)` 包；**幂等预声明与业务写入在同一个事务内**；所有 repo 调用用 `txCtx`）：
   a. **幂等记录预声明**（事务内第一条语句，借 `UNIQUE(user_id, idempotency_key)` 约束做单语句原子声明 + 同事务回滚兜底）：
      ...
      - **InnoDB 锁语义**（关键）：本 INSERT 会对 unique key `(user_id, idempotency_key)` 取**排他锁**；同 key 并发请求里只有一个事务能进入"持锁状态"，其他事务**在 INSERT 语句上阻塞**等待，直到首个事务 commit 或 rollback 释放锁。这是契约层对"同 key 并发不进双业务事务"的硬保证（**不**靠 MySQL 业务表行锁兜底——FOR UPDATE 在步骤 5c 才执行，已晚于幂等声明）。**r11 review 显式说明**：因 MVCC 让步骤 3 autocommit SELECT 看不到首事务的 pending 行，pending 阶段同 key 重试会"穿透"步骤 3 + 步骤 4 抵达本步骤 → 在本 INSERT 语句上**阻塞等待首事务释放 X-lock**（通常 < 100ms，覆盖业务事务时长），首事务 commit 后本事务拿到 `affected_rows = 0` 进入步骤 5b 短路读 success；首事务 rollback 后本事务拿到 `affected_rows = 1` 走全流程
   ...
   b. **短路分支**（命中已存在的成功行；**r11 review 锁定**：本路径下 status 必然是 `'success'` —— 首事务通过 X-lock 阻塞排队 + commit 推进到 success 与释放锁原子，本事务等到锁释放后看到的必然是 committed final 状态；理论上不存在 status='pending' 分支）：
      ...
      - `status = 'success'`（首次开箱已成功，`response_json` 已写入）→ COMMIT（本事务无写操作，COMMIT / ROLLBACK 等价）→ **反序列化 `response_json` + 补算 `data.nextChest.status = (unlock_at > now) ? 1 : 2` + `data.nextChest.remainingSeconds = max(0, ceil((nextChest.unlockAt - now) / 1s))`（两字段**同源同时刻**计算，用同一个 `now` 快照；r11 review 锁定：与步骤 3 cached 路径 + §7.1 GET /chest/current 完全对齐 —— 防止仅重算 `remainingSeconds` 而漏 `status` 导致重试发生在新 chest 已到期解锁时刻返回 `status=1` + `remainingSeconds=0` 不可能组合） + 填入当前请求的 `requestId`**（`response_json` 缓存**不**包含 `nextChest.status` / `nextChest.remainingSeconds` / `requestId`，由 server 在响应序列化时实时填入；详见步骤 5j `response_json` 字段选择说明）→ 返回 `200`
      - **理论上不会观察到 `status = 'pending'`**（r11 review 锁定）：InnoDB unique-key X-lock 让并发同 key 请求 serialize 排队 —— 步骤 5a 拿到 `affected_rows = 0` 时首事务必然已 commit 且 status 已推进到 `success`（commit / 锁释放 / status='success' 是同一事务的原子结束）。**保留语义兜底**：万一 driver 层 read-uncommitted bug 等极异常情况下读到 `status = 'pending'` → COMMIT → 返回 **1009 服务繁忙**（**非** 1008 —— 1008 在 r11 设计下不再有可达路径，详见错误码表 1008 / 1009 行 + 「1008 错误码 r11 退役决策」段）
...
7. **事务后处理**（无 post-tx Redis 写回 / 无 best-effort post-rollback failed upsert）：
   - **事务 commit 成功**（步骤 5a~5l 全部成功）→ idempotency 记录的 `status = 'success'` + `response_json`（**不含** `nextChest.status` / **不含** `nextChest.remainingSeconds` / **不含** `requestId`）已**原子**落盘；同 key 后续重试在步骤 3 committed success 幂等命中预检命中 `status = 'success'` 分支，反序列化 `response_json` + 重新按"当前时刻"补算 `nextChest.status = (unlock_at > now) ? 1 : 2` + `nextChest.remainingSeconds = max(0, ceil((unlock_at - now) / 1s))`（两字段**同源同时刻**计算保证内部一致；尤其重试发生在新 chest 已到期解锁的时刻会返回 `status = 2` + `remainingSeconds = 0`，与 §7.1 GET 同一秒查询完全对齐）+ 填入**当前**请求的 `requestId` 后返回，**无任何 Redis 依赖**、**无 stale status**、**无 stale 倒计时**、**无 stale requestId**；**重试请求绕过步骤 4 限频检查**（r10 review 锁定 / r11 review 限定仅 committed success replay 免配额，详见步骤 3 / 步骤 4 + 本节末「关键约束」「rate_limit 位置 r10 调整 + r11 修订」段）
   ...
   - **pending 阶段（首事务进行中）同 key 重试**（**r11 review 锁定**，**修订** r10 "pending 命中免配额" 承诺）：因 MVCC 让 autocommit SELECT 看不到首事务的 pending 行，pending 阶段同 key 重试在步骤 3 表现为"未命中"→ 步骤 4 限频通过则**会计入 quota**（与 r10 承诺不同 —— pending precheck 在 MVCC 下不可实现） → 步骤 5a INSERT 在 unique-key X-lock 上**阻塞等待首事务释放锁**（业务事务通常 < 100ms）→ 首事务 commit 后本事务拿 `affected_rows = 0` 走步骤 5b 短路读 success 返回 cached（业务正确性不受影响，仅多消耗 1 个 rate_limit 配额）；首事务 rollback 后本事务拿 `affected_rows = 1` 走全流程。**client 应使用合理退避策略**（首次重试间隔 ≥ 200ms 覆盖业务事务时长）避免在 pending 窗口内打满 quota；详见 §7.2 「client 重试策略」段
8. **响应**：返回 `{reward, stepAccount, nextChest}` 三段嵌套 payload（首次成功路径来自步骤 6 补算后的结果；committed success replay 路径来自步骤 3 反序列化 + 补算 `nextChest.status` 与 `nextChest.remainingSeconds`（同源同时刻，r11 review 锁定）+ 填入当前 `requestId`；步骤 5b cached 路径同步处理 —— 两条 cached 路径走完全相同的字段补算公式）
...
- **DB 持久化幂等的写后保证**（r7 review 修订 / r11 review 锁定：成功路径短路点重排到步骤 3，与 §7.2.3 / §7.2.5 配套）：
  - **成功路径**：步骤 5l commit 成功 → idempotency 行的 `status = 'success'` + `response_json`（**不含** `nextChest.status` / **不含** `nextChest.remainingSeconds` / **不含** `requestId`，避免回放 stale 状态 / stale 倒计时 / 破坏 trace 语义）已**原子**落盘 → 同 key 重试在**步骤 3 (committed success precheck)** 命中 `status = 'success'` 行 → 直接返回缓存的 `response_json` + **同时**补算 `nextChest.status` 和 `nextChest.remainingSeconds`（**同 now 快照**，同源同时刻）+ 填入**当前**请求的 `requestId` → 跳过步骤 4 rate_limit + 跳过步骤 5 业务事务；**零 Redis 依赖**、**零"SET 失败 client 卡死"风险**、**零回放 stale 状态**、**零回放 stale 倒计时**、**零回放 stale requestId**
...
- **rate_limit 位置 r10 调整 + r11 修订**（r10 review 锁定 "免配额免在 cached success replay 上"，r11 review **限定** "免配额仅对 committed success replay 生效；pending 阶段同 key 重试不可免配额，因 MVCC 不可见"）：
  ...
  - **r10 决策（方案 A）**：把 **rate_limit 检查从 middleware 层挪到 handler 内层**，**置于幂等命中预检之后**：
    ...
    - **handler 入口先做幂等命中预检**（步骤 3，autocommit SELECT chest_open_idempotency_records）：命中 `status = 'success'` → 返回 cached response（**跳过步骤 4 rate_limit**）；**未命中**（含真新请求 + pending 不可见的同 key 并发）→ 继续步骤 4
    - **rate_limit 仅对"未命中 committed success"的请求做**（步骤 4，**r11 review 修订**：原 r10 写 "只对真新请求做"，r11 收紧为"对未命中 committed success 的所有请求做"——包括 pending 阶段同 key 重试，因 MVCC 不可见）：按 user_id 60/min；超限 → 1005；未超 → 继续步骤 5 业务事务
  - **r11 review 锁定的 r10 设计缺陷**：r10 描述的"步骤 3 命中 `status = 'pending'` → 返回 1008 + 免 rate_limit"路径**不可实现**。原因 / 验证：(a) 首事务 BEGIN → INSERT pending row（尚未 commit）→ 此时 pending 行**仅对首事务自己的 read view 可见**；(b) 第二请求 autocommit SELECT WHERE user_id=? AND idempotency_key=? → InnoDB MVCC（默认 REPEATABLE READ）让 autocommit 的 read view **看不到首事务的 pending 行** → SELECT 返回空结果集 → 步骤 3 判定为"未命中"→ fall-through 到步骤 4 rate_limit + 步骤 5 业务事务 → INSERT 在 unique-key X-lock 上阻塞排队 → 首事务 commit 后第二事务拿 `affected_rows = 0` → 走步骤 5b 短路读 success 返回 cached。**问题**：此路径下第二请求已被步骤 4 计入 rate_limit quota，r10 关于"pending 命中免 rate_limit"的承诺**无法在 MySQL 默认隔离级别下实现**（除非把 SELECT 改成 `READ UNCOMMITTED` —— 但这会引入"事务 rollback 后第二请求误以为 commit"的更严重一致性 bug，**禁止**）
  - **r11 决策**：保留 r10 的"committed success replay 免 rate_limit"承诺（这是 server 端能保证的核心契约，client UX 受益最大）；**移除** r10 关于"pending 阶段同 key 重试免 rate_limit"的承诺；**pending 阶段同 key 重试**（首事务进行中，通常 < 100ms 窗口）**会消耗** rate_limit quota，client 应使用合理退避策略（首次重试间隔 ≥ 200ms 覆盖业务事务时长）避免在 pending 窗口内打满 quota。详见 §7.2「client 重试策略」段
  - **理由**：(a) committed success replay 路径 = 用户已有"首次成功落盘"事实 + client 仅做"读取已发放奖励"动作 + server 不消耗任何业务资源（无业务事务）→ 与 GET 类查询等价，不该消耗 rate_limit 配额 = 与"60/min 限频是对业务消费的节流"语义对齐；(b) pending 阶段同 key 重试 = MVCC 决定 server 端**无法识别**这是"同 key 重试"还是"真新请求"，故只能统一按"真新请求"路径处理（消耗 quota）—— 这是 r11 review 揭示的真实约束，非 r10 设想的逻辑选择问题；(c) 真新请求（未命中 / rollback 后到达）= server 进业务事务、占 chest / step_account FOR UPDATE 锁、走加权抽奖、INSERT chest_open_logs 等资源 → 必须计入 rate_limit 配额，否则被攻击者通过"频繁换新 idempotencyKey"绕过限频；(d) 同 key 重试 storm（如 client bug 退避策略错误）= 首次成功后所有同 key 重试都命中 committed success cached → 不计入配额，无 starvation 风险；首次失败后所有同 key 重试都走真新路径 → 计入配额，retry storm 在限频上有节流（详见步骤 7 「事务后处理」rollback 路径）；pending 阶段密集重试（client 退避策略 bug）= 单 pending 窗口内最多消耗 60/min 一部分 quota，业务正确性不受影响（步骤 5a X-lock 兜底）—— 这是 r11 接受的"次优契约"，相比 r10 不可实现的设想，是工程层最佳折中
  - **r10 决策不采纳方案 B 的理由**：方案 B（middleware 接受 cache-hit hint / response 完成时 decrement counter）有 race 风险（计数器在 middleware-pre 与 handler-post 之间有窗口）+ 需要修改 Story 4.5 钦定的 rate_limit middleware 接口，跨 epic 改动成本高于方案 A；r11 沿用此判断（方案 B 也无法解决 MVCC 不可见根因）
  - **跨接口影响**（Story 20.6 实装 + 节点 11 / Epic 32 复用启示）：本调整**仅对 §7.2 POST /chest/open 生效**，**不**影响节点 2 / 3 / 4 / 5 / 6 其他已认证业务路由（它们继续走 §4.5 钦定的统一 rate_limit middleware）；若节点 11 `POST /api/v1/compose/upgrade` 复用同一持久化 idempotency 模式（Story 32.4 锚定），**应**采用相同的"handler 内层 rate_limit + 仅免 committed success replay 配额"结构，**禁止**沿用 r5~r9 的"middleware 层 rate_limit + 重试消耗配额"老路径；**禁止**尝试"pending 阶段免配额"——MVCC 不可见在 MySQL 默认隔离级别下是硬约束
  - **实装层落点**：详见 Story 20.6 handler 实装；契约层只要求两条不变量 —— (i) 幂等命中预检（步骤 3）在 rate_limit 检查（步骤 4）之前；(ii) committed success replay 不计入配额。pending 阶段同 key 重试的配额消耗**不**作为契约不变量（实装层若未来引入"durable in-flight tracking 表"或类似机制让 pending 可见，可选择性免配额，但不强制）

- **MVCC 下 pending 不可见 r11 决策**（**r11 review 锁定** 协议层硬约束）：
  - **背景**：r10 设计在步骤 3 设了"命中 status='pending' → 返回 1008 + 免 rate_limit"路径，**假设** autocommit SELECT 能观察到首事务的 pending 行。
  - **r11 review 揭示的硬约束**：InnoDB 默认 REPEATABLE READ 隔离级别 + MVCC 实现下，autocommit SELECT 启动时取一个独立的 read view，该 view 仅包含"在 SELECT 启动时刻已 commit 的事务"的修改；**首事务的 INSERT pending 行尚未 commit，对 autocommit read view 不可见**（即使 autocommit 在物理时间上晚于 pending INSERT 的写入）。这是 InnoDB MVCC 的核心语义，非实装选择，**无法**在 MySQL 默认隔离级别下规避。
  - **不可选方案及理由**：
    - **降低隔离级别到 READ UNCOMMITTED**：可让 autocommit 看到 pending 行，但会同时让"首事务 rollback 后第二请求误以为已 commit"，引入更严重的"看到不存在的状态"bug。**禁止**
    - **改用应用层 in-memory 表（如 Redis）做 pending 跟踪**：与已采用的 MySQL 同事务幂等（r5~r9 决策）冲突 + 引入"应用层状态与 DB 状态不一致"风险（Redis 是非事务存储，r4 review 已驳回）。**禁止**
    - **首事务在 INSERT pending 后立即 commit pending 行（auto-commit-pending）**：要求把幂等记录拆出业务事务，回到 r5 设计 —— rollback 时 pending 行不跟着回滚，同 key 重试卡 1008（r6 review 锁定的悖论）。**禁止**
  - **r11 决策**：**接受 MVCC 不可见为协议层硬约束**；步骤 3 仅检查 committed success（这是 server 端能确定性观察到的状态）；pending 阶段同 key 重试在步骤 3 视为"未命中"，由步骤 5a 的 InnoDB unique-key X-lock 兜底（让并发请求 serialize 排队，首事务结束后第二请求自然分支到 success cached 或全流程）。这是 r11 review 锁定的根因解决路径，**禁止**未来 review 重新引入 "pending 命中预检"
  - **影响**：(a) 1008 错误码在新设计下**不再有可达路径**（详见下一段「1008 错误码 r11 退役决策」）；(b) pending 阶段同 key 重试消耗 rate_limit quota（详见上一段「rate_limit 位置 r10 调整 + r11 修订」）；(c) client 同 key 重试逻辑相应调整 —— pending 窗口内密集重试可能撞 1005，client 应使用 ≥ 200ms 退避（详见 §7.2「client 重试策略」段）

- **1008 错误码 r11 退役决策**（**r11 review 锁定** 节点 7 阶段本接口实际不可达，保留全局错误码定义供未来扩展）：
  - **背景**：r6~r10 设计在步骤 3 / 步骤 5b 保留 "status = 'pending' → 返回 1008" 路径，作为极窄 race 兜底。
  - **r11 review 锁定**：(a) 步骤 3 在 MVCC 下**不可能**观察到 pending 行（前述「MVCC 下 pending 不可见 r11 决策」段）→ 步骤 3 永远不会返回 1008；(b) 步骤 5b 在 X-lock serialize 排队下，第二事务拿到 `affected_rows = 0` 时首事务必然已 commit + status 已推进到 'success'（commit / 锁释放 / status='success' 是同一事务的原子结束）→ 步骤 5b 永远观察到 `status = 'success'`，不会观察到 `status = 'pending'`。因此 1008 在节点 7 阶段本接口**没有可达触发路径**。
...
- **client 重试策略**（r5 review 锁定 / r6 review 修订 / r7 review 简化 / **r11 review 调整 pending 阶段配额承诺**；**取代** r4 的"60s sentinel + 60s 外换新 key + UI 提示"分段方案；DB 同事务幂等 + InnoDB 锁阻塞排队让所有 client 重试路径**业务正确性始终安全**，仅 pending 阶段密集重试会消耗 quota）：
  - **网络层重试**（如 client 收到 5xx / timeout）→ 用**同** idempotencyKey 重试 + 指数退避（**r11 review 新增建议**：首次重试间隔 ≥ 200ms 覆盖业务事务时长，避开 pending 不可见窗口；**r10 review 锁定**：committed success replay 命中时**不**计入 rate_limit 配额，因此首次 commit 之后的同 key 重试不会触发 1005）；server 端处理路径分三类：
    - **committed success replay 路径**（首事务已 commit，server 端 idempotency 行 `status = 'success'` 已落盘）→ 步骤 3 命中 → **跳过步骤 4 限频** → 返回 200 + cached response（与首次结果一致；`nextChest.status` + `nextChest.remainingSeconds` 均按当前时刻**同源同时刻**实时补算 + `requestId` 填本次重试请求的 trace ID，不复用首次时刻的 stale 值与 stale trace ID —— 尤其重试发生在新 chest 已到期解锁的时刻会自然返回 `status = 2` + `remainingSeconds = 0`，与 §7.1 GET 同一秒查询完全对齐）。**重试间隔 ≥ 200ms** 时这是绝对安全路径（不消耗 quota / 不会撞 1005）
    - **pending 阶段同 key 重试**（首事务进行中 / 业务事务通常 < 100ms）→ **r11 review 锁定**：因 MVCC 让 autocommit SELECT 看不到首事务的 pending 行，步骤 3 表现为"未命中"→ 走步骤 4 **计入 rate_limit quota**（这是 r11 对 r10 "pending 命中免配额" 承诺的修订，详见「rate_limit 位置 r10 调整 + r11 修订」段）→ 步骤 5a INSERT 在 unique-key X-lock 上阻塞等待首事务释放锁 → 首事务 commit 后本事务拿 `affected_rows = 0` 走步骤 5b 短路读 success 返回 cached（业务正确性不受影响，仅多消耗 1 quota）；**client 应使用 ≥ 200ms 退避**避免 pending 窗口内密集重试打满 quota（极端 case：client 退避策略 bug 导致 1ms 间隔密集重试 → 单 60s 窗口最多消耗 60 quota → 撞 1005）
    - **rollback 后到达**（首事务已 rollback，pending 行自动消失）→ 步骤 3 未命中 → **走步骤 4 限频检查（本次计入配额，因事务已 rollback 是"真新请求"等价首次到达）** → 步骤 5a `affected_rows = 1` → 走全流程
  ...
  - **1005 重试**（**r11 review 新增**：在 pending 窗口内密集重试打满 quota 后撞限频）→ client 应**等限频窗口（60s）后再试同 key**；首事务通常已在该 60s 内 commit，重试时步骤 3 直接命中 committed success cached → 免 quota 返回 200。**禁止**立刻换新 key 重试（理由：首事务可能即将 commit / 已 commit；换新 key 会绕过 idempotency 直接进新事务 → 重复扣 1000 步 + 重复出箱）
  - **1008 重试**（**r11 review 锁定**：节点 7 阶段本接口不再有可达路径；保留语义供未来其他幂等接口复用）→ **理论上不会触发**；若 client 在节点 7 阶段实际收到 1008（说明有未预期路径，可能是 driver bug 或 server 实装错误），按"同 key 退避 ≥ 2s 重试 + log warn 给 server 端排查"处理
  - **1009 重试**（DB 异常 / 数据完整性异常 / 步数乐观锁冲突 / server 5xx / **r11 review 新增**：步骤 5b 极端兜底分支读到 status='pending'）→ client **可**用**同** idempotencyKey 或**新** idempotencyKey 退避重试（首次事务已回滚，pending 行已消失，两者都安全；同 key 路径在 5a `affected_rows = 1`，新 key 路径走新事务）；UI 无需"奖励可能已发放，请联系客服"等不确定文案（r4 的 60s 外重复出箱风险已通过 DB 同事务幂等彻底消除；r7 移除 best-effort failed upsert 后**不**会再出现"同 key 看到 failed 必须换 key"的语义负担）
  - **client 实装伪代码**（Story 20.6 / Epic 21 实装锚定；**r11 review 修订**：移除 1008 分支 + 新增 1005 退避路径 + 首次重试间隔 ≥ 200ms）：
    ```
    key = generateIdempotencyKey()
    retries = 0
    loop:
      resp = POST /chest/open {idempotencyKey: key}
      if resp.code == 0: return resp.data  // 成功 / committed success replay
      if resp.code == 1005 && retries < 2:
        // pending 窗口内密集重试打满 quota；等限频窗口后同 key 再试，首事务通常已 commit，命中 cached success
        sleep(60s + jitter)
        retries += 1
        continue  // 不换新 key（首事务可能已 commit；换新 key 会绕过 idempotency 重复扣步数）
      if resp.code == 1009 && retries < 2:
        // DB 异常 / 并发冲突 / server 5xx / 极端兜底分支；首次事务已 rollback、pending 行已消失，同 key 重试安全（也可选新 key，两者等价）
        sleep(min(1s * 2^retries, 4s))
        retries += 1
        continue
      // 1008 在节点 7 阶段本接口不可达（r11 review 锁定）；若实际收到则按 1009 路径处理 + log warn
      return resp.code  // 其他错误码按对应分支处理（3002 / 4001 / 4002 等）
    ```
    **关键参数**：**首次网络层重试间隔 ≥ 200ms**（覆盖业务事务 < 100ms 时长，避开 pending 窗口）；后续指数退避 1s / 2s / 4s（最多累计 7s）；撞 1005 时退避到下个限频窗口（60s + jitter）
...
- **TTL 与 Redis 决策 vs round 3 / round 4 / round 5 / round 6 / round 7 / round 9 / round 10 / round 11**（r11 修订）：
  ...
  - **round 7**：彻底**移除** best-effort post-rollback failed upsert（race condition 详见 r7 决策段）；schema 简化为 `status ENUM('pending', 'success')` 二态机；`response_json` 缓存**不**包含 `requestId`（顶层 trace ID，重试请求需重新填**当前**请求的 trace ID，不回放 stale 首次值）
  - **round 9**：把 `nextChest.status` 也加入"不持久化到 `response_json` + 回放时**同源同时刻**实时计算"列表（与 `remainingSeconds` 同处理），防止 stale `status=1` + 实时 `remainingSeconds=0` 不可能组合
  - **round 10**：把 rate_limit 检查从全局 middleware 挪到 handler 内层、置于幂等命中预检之后；**设计假设** committed success replay + pending 命中两类同 key 重试都免 quota
  - **round 11（本轮锁定）**：(a) **修正 r9 漏 status**：步骤 5b cached success 短路返回路径**同时**补算 `nextChest.status` 和 `nextChest.remainingSeconds`（同源同时刻），与步骤 3 + §7.1 GET 完全对齐 —— r9 仅锁定 `response_json` 缓存内容不含这两字段，但 r10 步骤 5b 短路返回路径文字只补 remainingSeconds 漏 status，本轮显式补齐两字段同处理；(b) **修正 r10 pending 免配额承诺**：MVCC 决定 autocommit SELECT 看不到首事务的 pending 行，r10 步骤 3 "pending 命中 → 1008 + 免配额" 路径**不可实现**；本轮整体重构步骤 3 仅检查 committed success + pending 阶段同 key 重试通过 X-lock 阻塞排队兜底；(c) **1008 错误码在节点 7 阶段本接口退役**（无可达触发路径），保留全局错误码定义供未来扩展；步骤 5b 兜底分支读到 pending 时返回 1009 而非 1008；(d) **client 重试策略更新**：首次网络层重试间隔 ≥ 200ms 覆盖业务事务时长避开 pending 窗口；撞 1005 时退避到下个限频窗口（60s）；接口对外承诺"committed success replay 免 quota + 业务正确性 100% 安全"，pending 阶段同 key 密集重试会消耗 quota 是 MVCC 硬约束下的"次优契约"
...
3. **按 cosmetic_item_id 聚合 + config 三态完整矩阵**：把步骤 2 结果按 `cosmetic_item_id` 分组；每组关联 `cosmetic_items` 拿配置元信息（`name / slot / rarity / icon_url / asset_url`）。**配置关联禁止用 `is_enabled = 1` 过滤**（关键约束：实例可见性与配置 enabled 状态完全解耦，详见关键约束「config 三态」段）。对每个聚合组，按其 `cosmetic_item_id` 在 `cosmetic_items` 表的命中情况落入以下**三态之一**（互斥且穷尽，实装时**必须**三态全覆盖，**禁止**只处理其中两态）：

   | 态 | 判定条件 | 该组是否出现在 `groups[]` | 元信息来源 | 日志 |
   |---|---|---|---|---|
   | **A. enabled** | `cosmetic_items` 行存在且 `is_enabled = 1` | **是** | row 真实值（正常 metadata，`iconUrl` / `assetUrl` 非空） | 无 |
   | **B. disabled-but-exists** | `cosmetic_items` 行存在但 `is_enabled = 0` | **是**（已拥有不得静默丢失） | **row 真实值（正常 metadata，与态 A 完全一致，含真实非空 `iconUrl` / `assetUrl`）**；**不**用 placeholder | 无（admin 下架配置是常规运维动作，不是数据治理事故，**不** log error） |
   | **C. missing-no-row** | `cosmetic_items` 表无匹配行（admin 物理删了 row，但用户**已拥有**实例仍在 `user_cosmetic_items`） | **是**（已拥有不得静默丢失） | 降级占位元信息（见下方「降级占位元信息」） | **log error**（数据治理事件） |

   - **保留可见性原则（覆盖态 B + 态 C）**：用户已拥有的实例（`status IN (1,2)`）**不得**因关联配置被 admin 禁用（态 B）或删除（态 C）而从 inventory 静默消失（= 用户可见数据丢失）。三态下该组都照常出现在 `groups[]`，`count` / `instances[]` 与正常组一致（实例数与实例 id 始终来自 `user_cosmetic_items`，与配置是否存在 / 是否 enabled 无关）。
   - **态 B metadata 判定理由（disabled-but-exists 用 row 真实值而非 placeholder）**：态 B 下 `cosmetic_items` row 物理存在，`name / slot / rarity / icon_url / asset_url` 真实数据**可得**，故直接用真实值组装，client 渲染体验与态 A 无差异（仅"是否可在 catalog / 抽奖中出现"不同，而 inventory 是"已拥有清单"语义，不受 enabled 影响）。这与 round 2 锁定的「enabled cosmetic 正常行 `iconUrl` / `assetUrl` 非空」契约**不冲突** —— 态 B 的 row 是 admin 写入过的合法配置（曾 enabled 或仍保留完整字段），其 `icon_url` / `asset_url` 在 §8.1 / §7.2 / Story 20.3 seed + admin 写入层约束下**仍非空**（DDL `DEFAULT ''` 仅兜底，业务层保证非空，与 §8.1 catalog 同源约束一致）；用 placeholder 反而会对"数据可得的已拥有项"做无谓信息降级，违背"不得静默丢失"的精神（丢失的不只是"该项存在"，还有"它原本长什么样"）。
   - **态 C 降级占位元信息**（**仅**态 C missing-no-row 使用）：`name = "未知装扮"`、`slot = 99`（other，§6.8 枚举内）、`rarity = 1`（common，§6.9 枚举内）、`iconUrl = ""`、`assetUrl = ""`。**这是空字符串 `iconUrl` / `assetUrl` 的唯一合法出现路径** —— 态 A / 态 B 组（含 §8.1 catalog 全量返回）的 `iconUrl` / `assetUrl` 必须非空，与 §7.2 `reward.iconUrl` / `reward.assetUrl` 冻结契约对齐（catalog / inventory 态 A·B 行与 chest-open 抽奖同源合法配置行，不能让其出现"永不可奖励"的空 URL 数据）；空串**仅**作为 admin **物理删** row 后已拥有实例的降级 placeholder（态 C 数据**不可得**才被迫降级，与态 B 数据可得有本质区别）。这些占位值均落在既有枚举 / 约束值域内，**不**扩展任何 schema；client 据此渲染"配置已下架的已拥有道具"占位卡，仍能看到持有数量。
   - **态 C log error**（非 warning）：物理删 row 意味着 admin 删了仍有用户持有的 cosmetic_items 行，是需运维介入的数据治理事件（应补 down-migration / 恢复配置 / 走废弃流程），日志级别为 error 以触发告警；态 B（仅 `is_enabled = 0`）是常规下架动作**不** log error。三态均**不**报 client 错误码（只读查询不因单组配置缺失 / 禁用整体失败 —— 见关键约束）。
4. **组装响应**：每组 = `{cosmeticItemId, name, slot, rarity, iconUrl, assetUrl, count, instances: [{userCosmeticItemId, status}]}`；`count` = 该组 instances 数组长度（含 status=1 与 status=2，**不**只算 in_bag）；态 C 组按步骤 3 降级占位规则填充元信息字段，态 A / 态 B 组用 row 真实值。
5. **确定性两级全序排序（契约必需，非可选优化）**：组装后**必须**对 `groups[]` 与每组 `instances[]` 两级都施加**确定性全序**（total order，跨请求稳定，不依赖 SQL / JOIN 迭代顺序）：
   - **`groups[]` 排序**：`ORDER BY rarity ASC, slot ASC, cosmeticItemId ASC`（与 §8.1 catalog `rarity ASC, slot ASC, id ASC` **同根因风格对齐** —— `cosmeticItemId` 即 `cosmetic_items.id` 字符串化同义，client 可复用 catalog 的排序心智模型；`cosmeticItemId ASC` 是决定性 tie-breaker，按数值升序，因同 (rarity, slot) 必有多配置）。**态 C missing-no-row 组**用降级占位的 `slot = 99` / `rarity = 1` 参与排序（占位值落在枚举内，排序仍全序确定，态 C 组按 `rarity=1, slot=99` 归位，再由 `cosmeticItemId ASC` 决定性区分）。
   - **每组 `instances[]` 排序**：`ORDER BY userCosmeticItemId ASC`（按 `user_cosmetic_items.id` 数值升序；该列是 §5.9 `BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT`，**全局唯一**，单值即决定性全序 tie-breaker，无需再补键）。`status` **不**参与排序（同组内 in_bag / equipped 混排不影响顺序确定性，因 `userCosmeticItemId` 已唯一全序）。
   - **理由（为何契约必需而非实装自由）**：Epic 24（Story 24.1 ~ 24.5）每次打开 Wardrobe Tab 都重新 GET inventory 渲染聚合 grid + 实例列表（Story 24.2 不缓存），若两级排序未钉死，相同库存数据会因 SQL / JOIN 迭代顺序在不同请求间重排，client grid / list 抖动，违背"client 可依赖列表顺序做 UI 渲染"。两级 tie-breaker 均到**唯一键级别**（`groups` 到 `cosmeticItemId`、`instances` 到 `userCosmeticItemId`），保证**全序唯一**（不存在两个不同元素排序相等的情况）。如未来移除任一级 tie-breaker 或改任一排序键，视为契约变更（走 §1 冻结流程，与 §8.1 catalog 排序契约同等级别）。
6. **响应**：返回排序后的 `{groups: [...]}`；空背包返回 `{groups: []}`；**不分页**。
7. **事务边界规则**：本接口**不**需要 MySQL 事务（只读查询；步骤 2 + 步骤 3 关联可用单 JOIN 或两次 SELECT，实装层 Story 23.4 决定，契约层不约束；但**无论实装用 JOIN 还是分步查**，步骤 5 两级排序是契约必需，**不**得依赖 DB 返回的天然顺序）。
...
| `iconUrl` | string | 必填 | 态 A enabled + 态 B disabled-but-exists 组：`1 ≤ length ≤ 255` **非空**（row 数据可得，用真实值；同 §8.1 理由，与 §7.2 `reward.iconUrl` 非空对齐）；**仅**态 C missing-no-row（admin 物理删 row）降级占位组允许空字符串 `""`（见服务端逻辑步骤 3 config 三态矩阵） | 装扮图标 URL；与 §8.1 `items[].iconUrl` 同义 |
| `assetUrl` | string | 必填 | 态 A enabled + 态 B disabled-but-exists 组：`1 ≤ length ≤ 255` **非空**（同 `iconUrl`，与 §7.2 `reward.assetUrl` 非空对齐）；**仅**态 C missing-no-row 降级占位组允许空字符串 `""` | 装扮资源 URL；与 §8.1 `items[].assetUrl` 同义 |
...
> **注**：本接口**不**触发 1002（GET 无 body / 无 query 参数）；用户背包为空**不**报错 —— 返回 `{groups: []}`（code = 0）；关联配置无论态 A enabled / 态 B disabled-but-exists / 态 C missing-no-row，**三态均保留该组、均不报错码**（态 B 用 row 真实值不 log，态 C 用降级占位 + log error；**不** skip，**不**报 1009 / 1003 —— 单组配置禁用 / 缺失不让只读 inventory 整体失败；详见服务端逻辑步骤 3 config 三态矩阵）。
...
- **已拥有实例可见性 + config 三态完整矩阵**契约：用户已拥有的 `user_cosmetic_items` 实例（`status IN (1,2)`）**永远**出现在 inventory，**禁止**因关联的 `cosmetic_items` 配置被 admin **禁用**（态 B `is_enabled = 0`）或**物理删除**（态 C 无 row）而 skip 该组（= 用户可见数据静默丢失）。§1 显式允许 admin 增删 `cosmetic_items`、§8.1 catalog / §7.2 chest 显式按 `is_enabled = 1` 过滤，故"已发放的 `user_cosmetic_items` 引用了 disabled / deleted 配置"是受支持的常态（不是异常脏数据）。契约保证：实例的存在性 / 数量 / 实例 id 只取决于 `user_cosmetic_items`，与配置是否存在 / 是否 enabled **完全解耦** —— **Story 23.4 实装关联 `cosmetic_items` 时禁止加 `is_enabled = 1` 过滤**（否则 disabled 配置下已拥有项被静默隐藏，违背本契约）。三态处理：态 A enabled → row 真实 metadata；态 B disabled-but-exists → **row 真实 metadata（与态 A 一致，不 placeholder，不 log，因 inventory 是"已拥有清单"语义、数据可得且 row 仍受 §8.1 / Story 20.3 非空约束）**；态 C missing-no-row → 步骤 3 降级占位元信息（值全落既有枚举值域内）+ log error 供运维告警。如未来改为"disabled / 缺失则 skip"或"则整体报错"，视为契约变更（前者是数据丢失回归，后者破坏只读接口对单组脏配置的容错性）。
...
- **`iconUrl` / `assetUrl` 非空字符串**契约：态 A enabled + 态 B disabled-but-exists 组的 `iconUrl` / `assetUrl` 必须 `1 ≤ length ≤ 255` 非空，与 §8.1 catalog + §7.2 `reward.iconUrl` / `reward.assetUrl` 非空冻结对齐（态 A·B 的 row 是 admin 写入过的合法配置，受 §8.1 / Story 20.3 seed + admin 写入层非空约束，DDL `DEFAULT ''` 仅兜底，业务层保证非空，不能让 chest-open 抽奖源同源数据出现"永不可奖励"的空 URL）。空字符串 `""` **仅**允许出现在态 C missing-no-row（admin 物理删 row、数据不可得）降级占位组（见服务端逻辑步骤 3），属 round-1 引入的 missing-item fallback 专用语义；**态 B 数据可得故必须用真实非空值，不得降级为空串**（与 round 2 [P2] 锁定的 enabled 行 URL 非空契约一致，r3 [P2] 把"非空范围"从"enabled 行"明确扩展到"态 B disabled-but-exists 行"，消除"disabled 行 URL 形态未定义"歧义）。如未来放宽为"态 A·B 组也允许空 URL"，视为契约变更（且会重新引入与 chest-open 冻结契约的跨接口矛盾，fix-review r2 [P2] 已收紧）。
- **确定性两级全序排序**契约（fix-review 23-1 r3 [P2]）：响应必须对 `groups[]` 与每组 `instances[]` **两级都施加确定性全序**，跨请求稳定（不依赖 SQL / JOIN 迭代顺序）。`groups[]`：`ORDER BY rarity ASC, slot ASC, cosmeticItemId ASC`（与 §8.1 catalog `rarity ASC, slot ASC, id ASC` 同根因风格对齐，便于 client 复用排序心智模型；`cosmeticItemId ASC` 为决定性 tie-breaker，态 C 组按占位 `rarity=1, slot=99` 归位仍全序确定）；`instances[]`：`ORDER BY userCosmeticItemId ASC`（`user_cosmetic_items.id` §5.9 全局唯一主键，单值即决定性全序 tie-breaker）。两级 tie-breaker 均到唯一键级别保证**全序唯一**（不存在排序相等的两元素）。Story 24（Story 24.1 ~ 24.5）每次打开 Wardrobe Tab 重新 GET 渲染聚合 grid + 实例列表（Story 24.2 不缓存），若排序未钉死则相同库存跨请求重排、client UI 抖动 —— 故两级排序是契约**必需**部分而非实装自由（与 §8.1 catalog 排序契约同等级别）。如未来移除任一级 tie-breaker 或改任一排序键，视为契约变更（走 §1 冻结流程）。
...
- 第一次成功，后续重复请求返回第一次结果（`status = 'success'`，server 直接返回缓存的 `response_json` + 实时**同源同时刻**补算时间派生字段 `nextChest.status` 与 `nextChest.remainingSeconds`（与 §7.1 GET /chest/current 一致；**r11 review 锁定**：步骤 3 committed success replay 路径 + 步骤 5b cached 路径**两条路径都必须**同时补算两字段，不能仅补 remainingSeconds 漏 status）+ 实时填入**当前**请求的顶层 `requestId`
- 第一次处理中（事务尚未提交），同 key 并发请求被 InnoDB unique-key X-lock 阻塞排队等待首个事务结束（**r11 review 锁定**：步骤 3 在 MVCC 下看不到 pending 行，pending 阶段同 key 重试在步骤 3 表现为"未命中"→ 走步骤 4 计入 rate_limit quota → 步骤 5a 在 X-lock 上阻塞 → 首事务结束后分支到 success cached 或全流程；**不返回 1008**，因 1008 在节点 7 阶段本接口已无可达路径）
...
**r7 review 锁定**：幂等记录使用 **MySQL 同事务持久化**（非 Redis 缓存）；**预声明 + 业务写入 + 最终化**全部在同一业务事务内原子提交；schema 为二态机 `status ENUM('pending', 'success')`。**r10 review 锁定**：rate_limit 检查在 handler 内层、幂等命中预检（步骤 3）之后做（步骤 4）；**r11 review 修订**：仅 committed success replay 不计入配额，pending 阶段同 key 重试因 MVCC 不可见**会**计入配额（步骤 5a 由 X-lock 阻塞排队兜底业务正确性）。详见数据库设计 §5.16 `chest_open_idempotency_records` 表 schema + V1接口设计 §7.2 服务端逻辑步骤 3 / 5a / 5b / 5k + 关键约束「r7 移除 best-effort failed upsert 决策」+「rate_limit 位置 r10 调整 + r11 修订」+「MVCC 下 pending 不可见 r11 决策」+「1008 错误码 r11 退役决策」段。
...
- `(user_id, idempotency_key)` UNIQUE 约束 → 原子声明 + 并发阻塞排队（InnoDB unique-key X-lock）；**r11 review 显式承担"pending 不可见兜底"角色** —— 因 MVCC 让 autocommit SELECT 看不到 pending 行，pending 阶段同 key 并发请求由本 UNIQUE 上的 X-lock serialize 排队（步骤 5a），首事务结束后第二请求自然分支到 success cached 或全流程
- `status` ENUM('pending', 'success') → **二态机**（r7 review 锁定，从 r6 的三态机简化）；`pending` 仅在事务持锁期间出现（对锁等待者不可见，对其他事务的 autocommit read view 也不可见 —— InnoDB MVCC + REPEATABLE READ 隔离级别决定），事务 commit 推进到 `success`，事务 rollback 让 pending 行消失（**无** `failed` 状态 —— 详见 §5.16 + V1接口设计 §7.2 r7 决策段）
- `response_json` JSON NULL → 缓存首次成功响应，**不**包含时间派生字段（`nextChest.status` 与 `nextChest.remainingSeconds` 均是 time-derived，由 server 在响应序列化时按"当前时刻"**同源同时刻**实时计算填入；r9 review 锁定 `nextChest.status` 与 `remainingSeconds` 同处理，**r11 review 进一步锁定**两字段补算必须发生在**所有** cached success 短路返回路径上（步骤 3 + 步骤 5b 共两条路径），缺一不可 —— 防止重试发生在新 chest 已到期解锁时刻回放 stale `status=1` + 实时 `remainingSeconds=0` 不可能组合）/ **不**包含顶层 `requestId`（每次请求独立 trace ID，重试请求需重新填**当前**请求的 trace ID 以维持 log / trace 关联语义，r7 review 锁定）
