...
4. **UPDATE**：`UPDATE pets SET current_state = ?, updated_at = NOW() WHERE id = ?`（按上一步查到的 pet.id）
   - DB 异常（driver / 网络 / 约束冲突等 `err != nil`）→ 返回 1009 服务繁忙
   - DB 无错误（`err == nil`）→ **不论 `RowsAffected` 是 0 还是 1 都视为成功**，进入步骤 5。理由：(a) `RowsAffected == 1` 是常规情形（state 实际改变 + `updated_at` 写新值）；(b) `RowsAffected == 0` 在 MySQL/GORM 语义下**主要触发条件是"WHERE 命中 1 行但所有列值与入参完全一致"** —— 即"同 user 同 state 重复上报"幂等场景，此时业务上必须返回 200 OK + code = 0（与本节"幂等"声明 + line 500 元信息表对齐）。**注**：本接口的 UPDATE 把 `updated_at = NOW()` 也写入，理论上即便 `current_state` 未变 `updated_at` 也会变 → MySQL 通常仍报 `RowsAffected == 1`；但 GORM/driver 在某些 client/server time-zone 边界 + 某些 MySQL 配置（如 `innodb_flush_log_at_trx_commit` 异常）下可能仍返回 0，service 层**不**依赖该值判断成功失败。**不**走 1003 资源不存在分支：步骤 3 SELECT 已确认 pet 行存在，从步骤 3 到步骤 4 之间的"被删"窗口在节点 5 阶段无业务路径触发（无 DELETE pets 的接口），且即便发生也属 DB 层不一致而非"用户无 pet" —— 应走 1009 路径并 log error，由运维侧排查
   - **以上规则**：1009 只触发于 `err != nil`；`err == nil` 一律视为成功（不再读 RowsAffected）
