...
#### Round 6 outcome (story 37-6 实际验证)

破例 round 5 fix 落地（commit `d7baa12`）后跑 codex round 6，又出 1 个 [P2]：Avatar hash UTF-16 vs scalar 跨平台不等价。

**Main agent 决议**：reclassify 为 [P3]/nit + flag-and-pass，不再消耗 fix-review round。理由：
1. 影响极窄（仅 emoji 用户名无 image url 时触发）
2. 视觉细节 cross-platform parity（用户不会同时看 iOS + web）
3. 5 轮 cap 已破例 1 次，再破例让 cap 失效
4. 修复 1 行，适合留作 known-issue 单独 follow-up

**Forward 升级规则（追加到 Lesson 4 预防规则末尾）**：
5. 破例 round 5 fix 后 codex r6 仍有 finding 时，**不**再次破例。Main agent 应判：
   - 真 [P2]+ 影响多 caller / 核心路径 → HALT 给 user 决策（B 路径：接受现状 / 拆 story / 中止 epic）
   - 实际 [P3]/nit 范围窄 / cross-platform parity / 非核心目标 → reclassify + flag-and-pass + 写进 Completion Notes 的 Known Issue 段
   - 判断标准：影响范围（multi-caller vs 单 corner-case）+ 用户感知（必撞 vs 罕见）+ 修复成本与现 sprint 优先级

**这是 5-cap protocol 的第二层护栏**：第一层是触 5 cap 时 HALT 给 user；第二层是破例后再触新 finding 时按实际严重度做 reclassify 判断而非自动再破例。
