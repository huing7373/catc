...
### 6. 展示待提交内容（不等待确认，直接进入步骤 7）

**不询问任何东西** —— 用户在步骤 2 已经做过分诊确认；commit message 在步骤 7 机械生成；lesson 内容已经在步骤 5 写好。这三处都已锁定语义，本步骤不再二次议事。
...
输出完毕**立即**进入步骤 7 commit，**不**询问 yes / 调整 / 取消。

**用户中断兜底**：如果用户在本步骤后需要修改 lesson 或回滚，他们会主动打断（Ctrl+C 或新一轮消息）；本命令不主动给出口。这是用户在 2026-04-24 明确要求的（"改一下命令，直接提交就好，不需要问我"）。
