...
每条 story 都会经过：
  create (如需) → dev → codex review → fix (最多 15 轮) → story-done
```
...
loop:
  ...
  5) 根据该 story 的当前状态 dispatch:
     ...
     case 'review':
       → 输出: "▶ 进入 review/fix 子循环（最多 15 轮 codex review）"
...
- **codex review 报 API quota error / 网络挂**：HALT 报告 + 不消耗 review_round 计数（让重启循环时不被 15 轮上限错杀）
