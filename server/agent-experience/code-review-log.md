...
## [1-5-profile-preferences-displayname-timezone-quiethours] Round 2 — 2026-04-19

| # | 类别 | 错误模式 | 文件 | 影响 |
|---|------|---------|------|------|
| 1 | patch | Round 1 在 `service.ProfileService.Update` 加 preflight 返回 `*dto.AppError{Code:"VALIDATION_ERROR"}`，但 `ws.ProfileHandler.HandleUpdate` 对 service err 只有 "ErrUserNotFound→INTERNAL_ERROR" 与 "default→`ErrInternalError.WithCause(err)`" 两个分支 —— 前者拦截存在性探测，后者把**所有**非 NotFound 错（包括 service 刚造出来的 AppError）统统再套一层 ErrInternalError。dispatcher 的 `errors.As` 命中最外层 AppError 后直接输出 `Code=INTERNAL_ERROR`，新的 preflight 永远无法把 VALIDATION_ERROR + 精确 message 送到客户端。 | server/internal/ws/profile_handler.go:78 | 上轮修复的客户端契约（Round 1 集成测试 `TestProfileUpdate_Integration_QuietHoursOnly_RejectedWhenTimezoneUnset` 断言 `out.Error.Code == "VALIDATION_ERROR"`）在真实线上完全不成立：底层持久层 bug（quietHours 静默失效）已修，但客户端看到 INTERNAL_ERROR，UI 无法给出"请先设置 timezone"的明确恢复路径，体验退回到"重试→重复失败"。修法：在 handler err 分支 **ErrUserNotFound 首先检查之后**追加 `var ae *dto.AppError; if errors.As(err, &ae) { return nil, ae }` 透传；顺序不能调：ErrUserNotFound 的存在性拦截必须跑在 AppError 透传之前，否则未来若 service 把 ErrUserNotFound 包进 AppError 就会泄漏存在性。新增 `TestProfileHandler_ServiceReturnsAppError_PassedThrough` 锁 VALIDATION_ERROR 透传契约 |

**构建验证：** ✅ `bash scripts/build.sh --test` 通过 + `go vet -tags=integration ./...` 通过

## [1-6-account-deletion-request] Round 1 — 2026-04-19

| # | 类别 | 错误模式 | 文件 | 影响 |
|---|------|---------|------|------|
| 1 | patch | Story 1.6 的 4 步删除流只吊销 refresh token + 断 WS 连接 + 清 resume cache，**没有任何层对 access token 做失效处理**。客户端手里那张 access token 还能继续打 `/v1/*` 鉴权端点（device.register / profile.update / account.delete 本身）最长 15 分钟，也能立刻重开 `/ws`（`ws.UpgradeHandler` 的 blacklist 原本为 Story 0.11 device ban 设计，deletion 路径根本没往里写）。「账号锁死」的窗口从文档声称的 `<100ms` 变成 access token TTL（默认 15 分钟）。业务下游任一 endpoint 不查 `users.deletion_requested` 就能继续正常处理被删用户的请求 —— 从合规 + 安全角度看这条 endpoint 等于只做了一半 | server/internal/service/account_deletion_service.go:169-183 | NFR-COMP-5 的「30 天起点」从 DELETE 那一刻起就应该把用户的 session 全部锁死；access-token 尚未失效期间用户可刷新 profile、注册 APNs token、收推送、发触碰、甚至在第二台设备新建 WS 连接 —— 每一条路径都在「已请求注销」的用户身上继续制造新状态 / 新副作用，违背删除请求的语义，Apple Privacy Policy 审核发现将直接拒审。修法：①service 加 `accountDeletionAccessBlacklister` 消费者接口（复用 `*redisx.RedisBlacklist`）+ Step 3 `blacklist.Add(userID, cfg.JWT.AccessExpirySec)`，**顺序在 Step 2 revoke 之后、Step 4 disconnect 之前** —— 避免「client 断 WS 后 reopen 和 blacklist 写入之间竞态」；fail-open with warn log (`account_deletion_access_blacklist_error`)；严格顺序单测从 4 步升 5 步 `mark→revoke→blacklist→disconnect→invalidate`。②middleware 加 `JWTAuthWithBlacklist(verifier, UserBlacklist)`，Verify 成功后查 `IsBlacklisted` —— 命中 `user_blacklisted` 返 401 AUTH_INVALID_IDENTITY_TOKEN（客户端清 Keychain 重 SIWA），Redis 错 fail-closed 返 500（ws/upgrade_handler 同策略）；`JWTAuth(v)` 保留做 nil-blacklist 反射性 wrapper 维持测试兼容。③wiring：`buildHTTPJWTAuth` 多收一个 blacklist 参数，生产挂 `JWTAuthWithBlacklist(jwtMgr, blacklist)`；`AccountDeletionService` 构造注入 `blacklist` + `time.Duration(cfg.JWT.AccessExpirySec)*time.Second`。④resurrection 兜底：`AuthService.SetAccessBlacklistRemover(AccessBlacklistRemover)` optional setter —— 不加 required ctor 参避免改动所有 NewAuthService 调用；SIWA resurrection path（ClearDeletion 成功后）fail-open 调 Remove，清空 blacklist，使 DELETE→SIWA<15min 重生用户的新 token 立即可用（不让 Apple reviewer 等 TTL）。3 个新单测锁 resurrection 分支（happy / RemoveErr fail-open / nil-remover 不 crash）。⑤集成测试：`HappyPath` 增断言 blacklist entry 存在；新增 `AccessTokenRejectedOn_V1_AfterDelete`（DELETE → 同 token 再 DELETE 401）+ `WSUpgradeRejectedAfterDelete`（403 DEVICE_BLACKLISTED）+ `ResurrectionAfterDeletion_WorksE2E` 追加断言 `userBL.TTL` 已 expired。`Idempotent_SecondCall_PreservesFirstTimestamp` 按「HTTP 层第二次 DELETE 需走 resurrection→新 timestamp」重写并显式注释语义；真正的 first-write-wins 锁仍在 repo 集成测试（`MarkDeletionRequested_IdempotentPreservesOriginalTimestamp`）里。|
