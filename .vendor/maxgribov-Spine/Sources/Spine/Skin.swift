...
class Skin {
    ...
    func texture(with name: String, from atlasName: String) -> SKTexture? {
        ...
        let requestedName = (name as NSString).lastPathComponent
        let textureName = atlas.textureNames.first { candidate in
            candidate == requestedName ||
            (candidate as NSString).deletingPathExtension == requestedName ||
            candidate == "\(requestedName).png"
        }

        guard let textureName else {
