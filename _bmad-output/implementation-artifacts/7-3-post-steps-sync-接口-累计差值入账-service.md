...
| 2026-05-03 | review r6 [P1+P2] fix：(1) [P1] r5 maxReported clamp 又破坏 reset 修复（reset 后基线永久卡死历史高水位）。reset 与"截断+乱序"无法用单一规则同时满足 → 选 **reset 优先** 路径回退到 r3 状态：service.SyncSteps rawDelta 改回 `clientTotal - latest.ClientTotalSteps`（仅 SUM 兜底作为第二层防御）；接受"截断+乱序"小幅多算作为 known limitation（损失上限 = single_sync_cap=5000 / daily_cap=50000 兜住，触发概率极低）。repo `MaxClientTotalStepsByUserAndDate` 方法保留供未来"sync_seq / device_id"额外信号接入复用，service 暂不调用。删除 `TruncationPlusOutOfOrder_MaxReportedClampPreventsOverAccrual` 单测 + 集成测试同名镜像（不再断言"完美防御"）；改写为 `KnownLimitation_TruncationPlusOutOfOrder_MayOverAccrue_ByDesign` 锁定 by-design 行为（accepted=4000，提示重蹈 r5 覆辙时立即挂）。(2) [P2] prod 配置覆盖未强制：service.NewStepService 新增 envName 参数（main.go 从 `CAT_ENV` env 读，默认 "prod"）；prod 严格策略下 cap 任意正值 → panic（safe-by-default：未注入/未知 env 都视为 prod）；新增 `NewStepService_ProdEnv_RejectsYAMLCapOverride` 单测覆盖 6+5 个 subtest。lesson `docs/lessons/2026-05-04-step-sync-r6-reset-vs-ooo-tradeoff-and-prod-env-gate.md`。build.sh --test + build.sh --integration 全绿；status 仍 review | fix-review workflow（epic-loop sub-agent） |
