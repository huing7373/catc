...
- **【AC 偏差登记 — fix-review 26-5 r3，wontfix-with-rationale】** `epics.md` §26.5 AC 并发1（行 3608）措辞「同一 pet 同一 slot 100 个并发 equip 不同实例 → **只 1 个成功，其他 99 个返回错误**（DB UNIQUE(pet_id, slot) 兜底）」与 **26-1 冻结契约 §8.3 同槽自动换装（swap）语义**冲突：26-1 §8.3 + 26-3 实装把 equip 钦定为「查旧 → 删旧 user_pet_equips 行 + 旧实例回 in_bag → 插新行 + 新实例 equipped」的 swap（client 无需先 unequip），首个 tx commit 后被串行化的后续 equip **设计上就该成功**。codex r3 **实跑实证**（`go test ./internal/service -tags=integration -count=3 -run ...Concurrent100SamePetSlot...` 报告 **91/100 成功 equip**，非 1）。AC 行 3608 括注「DB UNIQUE(pet_id, slot) 兜底」真正保证的是「**任意时刻至多 1 行 / 终态恰 1 行**」（= 终态一致性矩阵在断言的），**不是**「99 调用失败」；其「只 1 成功」措辞基于 26-1 冻结契约**之前的过时心智模型**（误以为 equip 是 insert-only 靠 uk_pet_slot 拒重复）。**处置（wontfix-with-rationale）**：本 story 范围红线禁止改生产代码 / 冻结契约 / AC 文档（26-5 只加测试），故 (1) **不改** epics.md AC 措辞（视为被 26-1 冻结契约 supersede）；(2) **不改** 服务（91/100 swap 成功是符合冻结契约的正确行为）；(3) 并发1 断言按 26-1 冻结契约 swap 语义实现 —— 成功数仅断言 `>= 1`（不设上界、不逐个断言失败错误码），核心真不变量为**终态一致性矩阵**（(pet,slot) 终态恰 1 行 / 恰 1 status=2 / 其余 N-1 status=1 / 行↔状态对齐 / 双向一致）。这是已确诊跑满 3 轮的 over-correction chain（r0 `==1` → r1 放松 `>=1`+终态矩阵 → r2 误恢复 `==1`+99 个 1009 逐个断言 → r3 codex 实证 91/100 证伪 r2 → 本轮回 `>=1` 收敛终结）；详见 lesson `docs/lessons/2026-05-18-concurrent-pseudo-invariant-vs-frozen-contract-swap-semantics-26-5-r3.md`。**禁止**未来把成功数断言再收紧回 `==1`（伪不变量，已被实证 3 次否定）。
