...
### Open Issues / Tech Debt（2026-05-04 review round 5 wontfix surface）

> ⚠️ **Spec gap surfaced by codex review round 5 P1**：fresh install 路径上没有任何代码调用
> `MotionProvider.requestPermission()`，导致 `bind(motionProvider:)` 的 `(false, false)` 象限永远命中
> 持引用 noop 路径 → `petState` 卡 `.rest` 永远 → 本 story 的 sprite 切换 feature 对 fresh install
> 用户**实际未启用**.
>
> **现状是 by design**（不是 bug）：本 story 的 spec 红线明确钦定"HomeViewModel 不调
> requestPermission，权限申请由 8.5 / 8.6 / 8.1 等后续 story 统一处理"（详见本文件 AC1 行 80
> "**不**调 motionProvider.requestPermission" + `HomeViewModel.swift:361-367` 红线 docstring）.
> 选择不在 8.4 修复 codex P1，是为了不破坏"按 story 顺序推进"的 spec 约束.
>
> **但这是真问题，必须升级为 epic 级 follow-up**：
>
> - **现状**：epic 8 的 sprint-status.yaml 只有 8-1（HealthKit）/ 8-2（CoreMotion）/ 8-3（mapper）/
>   8-4（本 story）/ 8-5（步数同步触发器）；其中 8-5 是"步数同步"业务，**不**涉及 motion 权限申请.
>   8-1 的"HealthKit 接入"已 done 但只管 `HKHealthStore.requestAuthorization`，与 CoreMotion
>   独立两条权限（`NSMotionUsageDescription` 在 Info.plist 单独配置；`CMMotionActivityManager`
>   首次 startUpdates 才隐式触发系统弹窗，**或**走主动 `CMMotionActivityManager`
>   + `queryActivityStarting:to:to:withHandler:` 一次性 1-event probe 显式预申请——MotionProviderImpl
>   已实装后者作为 `requestPermission()`，但**没生产 caller**）.
>
> - **本质**：epic 8 的切片设计**漏了一个 story** 来调 `motionProvider.requestPermission()`. 节点 3
>   demo 验收（Story 25.2 节点 8 demo 验收 / Story 9.x 跨端集成）跑 fresh install 时会暴露此 gap.
>
> **建议路径**（任选其一，需 SM / PM 决策；**不**在本 story 范围处理）：
>
> 1. **新建 Story 8-6 "首启动流程权限申请统一入口"**：在 RootView .task 启动期或第一次进 HomeView
>    时调 `await container.motionProvider.requestPermission()`，授权完成后调 RootView 已存在的
>    motionProvider rebind hook（`scenePhase == .active` 路径）让 8.4 升级到 startUpdates；可顺
>    带把 HealthKit 权限申请也并到此 story.
>
> 2. **挪到 epic 9（"跨端集成测试与 demo"）下作为 ScenePhase rebind 链路的扩展**：epic 9 节点 3
>    demo 验收时新加权限申请覆盖.
>
> 3. **Epic 8 retrospective 时立项**：epic-8-retrospective story 已在 sprint-status.yaml 列为
>    optional；retro 时显式 surface 此 gap 并决定开 8-6 还是合并到其他 epic.
>
> **代码侧不做改动**——保持 8.4 红线一致；本条目仅作为 surface 痕迹，不让 P1 静默漏到节点 3 demo.
>
> **lesson 沉淀**：详见 `docs/lessons/2026-05-04-auth-gated-feature-needs-explicit-requester-story.md`
> （epic 切片不能让"权限申请 caller"的 spec gap 漏到末端 story；surface 为 tech debt 比静默修代码
> 更负责任）.
...
- **Unit Tests**: `bash iphone/scripts/build.sh --test` ✅ **388 tests, 0 failures**（含 15 个 HomeViewModelMotionTests case 全 pass；round 1/4/5 共追加 8 case）
...
| 2026-05-05 | review round 5 fix: P2 motion handler stale write race（generation token 拦截 in-flight Task）；P1 wontfix surface 为 tech debt（epic 8 缺 requestPermission caller story） | 8.4 | claude-opus-4-7[1m] |
