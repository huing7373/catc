...
- **日期**：2026-04-29（v2 起草）/ 2026-04-29（v2.1-v2.4 修订）/ 2026-04-30（v2.5 X1+X2 修订）
...
## v2.4 → v2.5 修订摘要（用户 Architect review 第 4 项反馈：要"彻底"不要"妥协"）

用户在 Architect review 第 4 项（Story 2.3 / 5.5 partial revert 风险）指出：「我希望的是做彻底的修改，而不是妥协」。X1+X2 修订把"partial revert + 5 条 risk 缓解"路径改写为"completely supersedes + 重写"路径：

| 改动项 | v2.4 写法 | v2.5 写法 |
|---|---|---|
| ADR-0009 / ADR-0010 §4.1 措辞 | "对 Story 2.3 / 5.5 的影响：partial revert" | "对 Story 2.3 / 5.5 的影响：**completely supersedes** 主入口部分 / 数据持有部分（不是 partial revert）" |
| Story 2.3 / 5.5 sprint-status 状态 | done（不改）+ ADR 内文字声明 partial revert | **`superseded`**（done 的 commit 保留作历史，但其部分钦定不再 active；引用 ADR 注释）|
| Story 37.3 / 37.4 AC 风格 | "迁移现有 RootView/HomeViewModel + grep 全 codebase 引用兜底 + accessibility identifier 不丢失保证" | "**重新实装**：旧字段/路由整段直接删除；caller 漏改靠**编译器报错**驱动；功能完整性靠 **UITest 全链路覆盖**兜底" |
| §6 Risk Register | 14 条（含 #1 Story 2.3 partial revert 漏改、#2 Story 5.5 漏改、#7 ping/version 风险、#14 git blame 影响）| 11 条（删 #1 / #2 / #14；#7 升级为 Story 37.3 / 37.7 的 UITest 必含验收项；保留 #11 / #12 / #13 标 M（已闭环））|

**工程实质**：v2.4 → v2.5 不改最终代码结果（dev 写出来的 Swift 代码几乎一致），区别在**工程指令风格** + **commit 粒度** + **未来读者心智**：v2.5 让 dev 拿到的是"砍 + 重建"指令而非"小心迁移"，让未来读 ADR / sprint-status / Story AC 的人一眼明白"那部分钦定已死"，避免"妥协 + 缓解"措辞误导。

**git history 不可逆**：Story 2.3 / 5.5 已 done 的 commit 仍在 git history 里（commit 不可逆 + 远端已存）；v2.5 在 sprint-status.yaml 用 `superseded` 状态明确这部分钦定已不 active。
...
| Epic | 影响 | 处理 |
|---|---|---|
| Epic 1-5（已 done） | Story 2.3 + Story 5.5 部分钦定 dead（git history 保留；sprint-status.yaml 状态改 superseded）| **completely superseded** via ADR-0009 / 0010 |
...
| **Story 2.3 钦定 NavigationStack + Sheet 主入口** | **completely superseded via ADR-0009**（主入口部分钦定 dead；新 MainTabView 重新实装）；次级 sheet 场景作为 ADR 内独立决策延续保留 |
| **Story 5.5 钦定 LoadHomeUseCase → HomeViewModel.homeData** | **completely superseded via ADR-0010**（数据持有部分钦定 dead；HomeViewModel.homeData 字段直接删除）；LoadHomeUseCase / HomeRepository 接口作为独立决策延续保留 |
...
| 重构代码 | RootView .ready 分支 / AppCoordinator / HomeView / HomeViewModel **重新实装**（旧字段/路由 delete + 新代码 create；ADR-0009/0010 completely supersedes 部分钦定） |
...
> 注：`epic-37-retrospective: required`（不是 optional）—— 含 2 ADR + 主入口/数据流重新实装（completely supersedes Story 2.3/5.5 部分钦定），需要 retro 总结教训。
...
> **2026-04-30 X1+X2 修订**：删除 #1 / #2 / #14 三条迁移类风险（"重新实装"路径不存在；ADR-0009/0010 已 supersedes 而非 partial revert，旧代码直接 delete + 新代码 create，编译器报错驱动 caller 修复）；调整 #7 从"风险"改"UITest 必含验收项"。
...
| 10 | 微信绑定 UI 视觉壳 vs PRD「不实现 UI」边界争议 | L | PRD §4 澄清段加注「视觉占位属结构预留的一部分，行为不做」；Story 37.11 AC 明列「按钮触发后仅 toast」；PM 签字位（§8.2）必签 |
| 11 | currentTab 所有权冲突（v2 中 ADR-0009 / 0010 互斥） | M（已闭环）| v2.1 修订：currentTab 归 **AppCoordinator**（与 presentedSheet 同级），不进 AppState；两个 ADR 已同步 |
...
**UITest 必含验收项**（从原 #7 升级 —— 不再作为"风险"，而是 Story 37.3 / 37.7 done 的硬条件）：
- 启动 → mock server 收到 **1 次 `/ping`** + **1 次 `/version`** + **1 次 `/home`**（Story 2.5 + Story 5.5 已 done 的链路在 Story 37.3 重新实装的 RootView 内仍生效）
- 4 Tab 切换 → 内容更换 + 当前 Tab 高亮
- Home Tab idle → TeamIdleCard 创建/加入按钮可定位
- Home Tab inRoom → RoomView 出现 + 离开按钮可定位
- ResetIdentityViewModel.resetTapped → appState.reset() + Keychain 清空 + 重新 bootstrap 全链路通过
...
| 角色 | 责任 | 签字位 |
|---|---|---|
| **Architect（Winston）** | review ADR-0009 / ADR-0010 决策合理性；尤其 Story 37.3 / 37.4 的**重新实装路径**（completely supersedes Story 2.3 / 5.5 部分钦定，编译器报错驱动 caller 修复 + UITest 全链路兜底，不依赖 grep）；currentTab 归 AppCoordinator + ViewModel 构造注入规则确认；roomId String 类型契约确认 | **[x] 日期：2026-04-30 签字：huing**（4 项全 yes：currentTab 归 AppCoordinator ✅ / ViewModel 构造注入禁 @EnvironmentObject ✅ / roomId String? 贴 AR21 ✅ / Story 2.3+5.5 重新实装路径 ✅）|
| **Product Manager（John）** | review PRD §4 4 Tab 澄清段；**特别签字**：微信绑定 UI 视觉壳（按钮 toast）是否突破 §4「微信绑定（结构预留，节点 2 不实现 UI）」边界——v2.1 解释为「视觉占位属结构预留的一部分」是扩义而非纯澄清，需 PM 明确 yes/no | **[x] 日期：2026-04-30 签字：huing**（**yes，接受扩义**：Story 37.11 做微信绑定卡 + 绑定 Modal 视觉壳 + bell/settings 顶部按钮，按钮触发后仅 toast「微信绑定（敬请期待）」；产品考虑—提前露出"数据保护"产品价值话术 + 未来 epic 接 OAuth 时 UI 已就绪。**PRD §4 边界扩义已记录**：视觉占位 + 行为为空 = 仍属"结构预留"范畴）|
...
- [x] **用户终审通过日期**：2026-04-30
- [x] **落盘 commit hash**：见 git log（v2 主体 commit `fd0a8c2` + ui_design `c453525`；v2.5 X1+X2 修订 commit 见本次 commit）
- [x] **落盘 commit message**：参见 git log
- [ ] 第一条 story（37.1）启动日期：____（等本 commit 后跑 `bmad-create-story 37.1` 即填）
- [x] **codex 审本 v2 verdict**：v1 Reject(6B) → v2 Reject(3B+5W+2P) → v2.1 Reject(3B+2W+5P) → v2.2 Reject(2B+1W+4P) → v2.3 Reject(1B+2W+1P) → v2.4 **Accept** → v2.5 X1+X2 修订 → **Accept with revisions**（仅"叙事一致性"残留 partial revert 已 v2.5 末波清扫）

### 9.1 Architect 签字 review 回溯（2026-04-30）

| 项 | 决议 | 备注 |
|---|---|---|
| 1 · currentTab 归 AppCoordinator | ✅ y | TabView selection binding 走 `$coordinator.currentTab`；与 presentedSheet 同级；不进 AppState |
| 2 · ViewModel 构造注入 + 禁 @EnvironmentObject | ✅ y | ADR-0010 §3.1 加 ADR 级硬规则；View 层可用，ViewModel 层禁 |
| 3 · roomId 类型 String?（贴 AR21） | ✅ y | 0 server 改动；与 AR21 ID 字符串约定 + epics.md L2016 server /home `room.currentRoomId` 字符串契约对齐；不预设数字内容 |
| 4 · Story 2.3 / 5.5 重新实装路径 | ✅ y | X1+X2 修订：partial revert → completely supersedes；旧路由/字段直接 delete + 新代码 create；编译器报错驱动；UITest 全链路兜底 |

### 9.2 PM 签字 review 回溯（2026-04-30）

| 项 | 决议 | 备注 |
|---|---|---|
| 微信绑定 UI 视觉壳（PRD §4 边界扩义） | ✅ y | Story 37.11 做视觉壳（含 bell/settings + 微信绑定卡 + 绑定 Modal），按钮 toast「敬请期待」，不调真 OAuth；PRD §4「不实现 UI」边界扩义为"视觉占位 + 行为为空 = 结构预留"；未来 OAuth 真做的 epic 仅接 server，UI 已就绪 |
