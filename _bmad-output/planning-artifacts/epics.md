...
**执行顺序**：4.1 → 4.2 → 4.3 → 4.4 → 4.5 → 4.6 → **4.8** → **4.7**（Layer 2 集成测试是收尾性 story，放 epic 末尾执行；sprint-status.yaml 中已按此顺序排列，原占位 Story 4.9 已删除）。
...
> **2026-04-30 变更**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) 落地）：**前置依赖** Epic 37（iPhone 架构层重构 + UI Scaffold）需先 done。本 epic 启动时主入口已是 4 Tab + Home Tab 互斥（ADR-0009）+ AppState 单 source of truth（ADR-0010）。demo 验收基线变更：从「3 CTA + Sheet」改为「4 Tab + AppState 数据流 + ping/version + 自动登录」。
...
> **2026-04-30 变更**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) §5.1 落地）：原范围「房间页 SwiftUI 骨架」由 Story 37.8 RoomView Scaffold 完成；本 story 缩窄为「在 Story 37.8 已交付的 RoomView Scaffold 上把 MockRoomViewModel 替换为 RealRoomViewModel（持 WSState + members + memberPetStates，roomId 从 AppState 读 String 类型，按 AR21 ID 字符串约定）」。删除原 AC 中已由 37.8 完成的「成员列表 4 格占位」「房间号显示」「退出房间按钮」等项；保留「单元测试 ≥3 case」+ 「UI 测试」结构；房间号 label accessibility identifier 改为 `roomIdDisplay`。详细新版 acceptance 见提案 v2 §5.1。
...
> **2026-04-30 变更**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) §5.2 落地）：原范围「主界面入口完善（Sheet 模式 + 选项弹层 + "返回房间 #xxxx"按钮文案）」整段作废，由 Story 37.3 MainTabView + Story 37.7 HomeView TeamIdleCard + Story 37.12 JoinRoomModal + AppState 互斥状态机承担入口；本 story 缩窄为「3 个 UseCase 实装 + 写 AppState」。
> - CreateRoomUseCase 由 HomeView idle 态 TeamIdleCard"创建队伍"按钮触发，成功后写 `appState.currentRoomId: String?` → HomeContainerView 自动切 RoomView
> - JoinRoomUseCase（参数 `roomId: String`，AR21 ID 字符串约定，**不预设数字内容**）由 JoinRoomModal "确定加入"或 FriendsView "加入"按钮触发
> - LeaveRoomUseCase 由 RoomView "离开房间"按钮触发，写 `appState.currentRoomId = nil`
> 详细新版 acceptance 见提案 v2 §5.2。
...
> **2026-04-30 变更**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) §5.3 落地）：Given 段加 Story 37.7 + 37.4 前置；ChestCardView 通过 Story 37.7 HomeView 暴露的 `chestSlot: () -> ChestSlot` ViewBuilder closure 注入（HomeView 内部代码 zero edit）；HomeViewModel 仅持 `chestRemainingSeconds: Int`（Timer 驱动），`currentChest` 来自构造注入的 AppState（不允许 ViewModel 内 @EnvironmentObject，详见 ADR-0010 §3.1）。其它 AC 不变。
...
> **2026-04-30 变更**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) §5.4 落地）：原范围「Sheet 弹 InventoryView」由 ADR-0009 推翻 Sheet 主入口模式；现 Wardrobe Tab 直接路由（Story 37.3 MainTabView）+ Story 37.9 WardrobeView Scaffold 已交付分类 Tab / 3 列网格 / 预览区 + WardrobeViewModel 基类 + MockWardrobeViewModel 子类。本 story 缩窄为「实装 RealWardrobeViewModel: WardrobeViewModel 基类，inventory 来自 AppState（hydrate 来自 LoadInventoryUseCase）；selectedCategory / selectedCosmeticId 仍为 view-specific transient」。删除「Sheet 弹 InventoryView」语义。详细新版 acceptance 见提案 v2 §5.4。
...
### Story 24.4: 主界面"仓库"按钮入口完善（**2026-04-30 作废**）

> **2026-04-30 整条作废**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) §5.4 + ADR-0009 落地）：ADR-0009 推翻 Sheet 主入口模式，Wardrobe 改为 4 Tab 之一（MainTabView 直接路由），不再需要"主界面入口"按钮。功能由 Story 37.3 MainTabView 的 Wardrobe Tab 承担。`sprint-status.yaml` 内本 story status 改为 deleted。

(以下原 acceptance 仅作历史保留，不再作为 active spec)
...
> **2026-04-30 变更**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) §5.5 落地）：原范围「主界面 Sheet 弹合成页」改为「Wardrobe Tab 内 NavigationLink push」（ADR-0009 推翻 Sheet 主入口）。WardrobeView 顶部加"合成"小按钮（Story 37.9 已预留入口位置；本 story 实装真实 push 行为）；其它 AC 不变。
...
### Story 33.6: 主界面"合成"按钮入口完善（**2026-04-30 作废**）

> **2026-04-30 整条作废**（[sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) §5.5 + ADR-0009 落地）：ADR-0009 推翻 Sheet 主入口模式，合成入口改为 Wardrobe Tab 内 NavigationLink push（Story 33.1 落地）；不再需要"主界面入口"按钮。`sprint-status.yaml` 内本 story status 改为 deleted。
...
> **2026-04-30 落地**（依据 [sprint-change-proposal-2026-04-29-v2.md](sprint-change-proposal-2026-04-29-v2.md) v2.4）
>
...
>
> **核心 AC 红线**（所有 Scaffold story 共性，37.3/37.4 重构 story 不适用）：
> - 数据：完全 mock（构造注入 MockAppState 或 PreviewProvider）
> - API：禁止 import APIClient / Repository / UseCase（Story 37.13 含静态校验 grep 脚本）
> - 视觉：像素级匹配 `iphone/ui_design/README.md` §Design Tokens
> - 主题：用 `@Environment(\.theme)` 取 token
> - 测试：每 View 至少一个 SwiftUI Preview；关键交互覆盖 a11y identifier；**不引 SnapshotTesting / ViewInspector**（严守 [ADR-0002 §3.1](../implementation-artifacts/decisions/0002-ios-stack.md)）
> - 通过 `bash iphone/scripts/build.sh --test`
>
> **接缝设计**（codex review 强制）：每屏 ViewModel 走 class 层次结构（基类 ObservableObject + 子类 Mock/Real）；ViewModel 仅允许构造注入 AppState（不允许 @EnvironmentObject 反模式，详见 [ADR-0010 §3.1](../implementation-artifacts/decisions/0010-iphone-app-state.md)）。
...
**Given** Sprint Change Proposal v2.4 已用户终审通过 + ADR-0009 草稿已落盘
**When** 完成本 story
...
**Given** Sprint Change Proposal v2.4 已用户终审通过 + ADR-0010 草稿已落盘
**When** 完成本 story
**Then** ADR-0010 status 从 Proposed 改为 Accepted
**And** §3.1 ViewModel 注入规则 ADR 级硬规则 + §3.2 AppState 范围白名单（含 currentRoomId: String? 类型）+ §3.5 ViewModel 演变模式表覆盖现有 + 计划中 ViewModel（含 ResetIdentityViewModel / LaunchingViewModel）
**And** §6 验收 5 条全部勾选
...
- `ThemeColors.swift` / `ThemeSpacing.swift` / `ThemeRadius.swift` / `ThemeShadow.swift` / `ThemeTypography.swift`：分别覆盖 ui_design/README §Design Tokens 五类
...
As an iOS 开发,
I want 一组从 ui_design/components/primitives.jsx 翻译来的 SwiftUI 共享 primitives + 完整 25 键 Icons → SF Symbol 对照表,
So that 5 个 Feature View 复用统一的卡片 / 按钮 / 头像 / 渐入动效 / 稀有度色条 / 图标.

**Acceptance Criteria:**

**Given** Story 37.5 Theme 已就绪
**When** 完成本 story
...
- `PrimaryButton.swift`: 圆药丸（高度一半圆角）+ theme.colors.accent 底色 + 立体硬阴影 `0 4px 0 accent-deep` + 按下 `translateY(2)` 动效；支持 disabled 态
...
- `RarityTag.swift`: 接受 `Rarity { N, R, SR, SSR }` enum，按 ui_design/README §Wardrobe 配色 (N=灰 #b0b0b0 / R=蓝 #7db3e8 / SR=紫 #c58ae8 / SSR=金红渐变) 渲染色条
- `Icons.swift`: 含 `enum Icons { static let mapping: [String: String] = [...] }`，**1:1 对齐 ui_design/source/components/primitives.jsx 内 Icons 对象真实 25 键**（详见提案 v2 §7 Story 37.6 Icons 完整映射表）；未匹配键 log warning + 退回 `questionmark.circle`
**And** 每个 primitive 有 `#Preview` 块，candy 主题 + dark 主题各一个预览
**And** **单元测试覆盖**（≥3 case）:
- happy: `Icons.mapping["home"] == "house.fill"` + `Icons.mapping.count == 25`
- happy: 全 25 键 forEach `XCTAssertNotNil(UIImage(systemName: ...))` 验证 iOS 17+ 都存在
...
- 小猫舞台 Card：`Image(systemName: "cat.fill")` + 椭圆背景 + theme.colors.accent-soft 条纹 overlay + 等级名牌 + 三状态条
- ActionRow：3 个互动按钮（喂食 🍥 / 抚摸 💕 / 玩耍 ⭐）；点击触发 emoji floatUp 1.4s
- **宝箱位接缝**（**硬接缝**）：HomeView 是 generic struct `HomeView<ChestSlot: View>`，含 `let chestSlot: () -> ChestSlot` ViewBuilder closure 参数 + `@ObservedObject var state: HomeViewModel`（基类直接，不再泛型 state）；本期 Scaffold 阶段调用方传入 `EmptyView()` 占位；Story 21.1 时调用方改传入 ChestCardView，**HomeView 内部代码 zero edit**
- 底部 TeamIdleCard：theme.colors.accent 渐变 + Card 圆角 22 + "创建队伍" / "加入队伍" 两 PrimaryButton
- **接缝设计**（class 层次结构，禁 protocol any 模式）：
  - `class HomeViewModel: ObservableObject` 基类含 `@Published var greeting: String / weather / stats / interactionAnimation / showJoinModal: Bool` + abstract method `func onCreateTap() / onJoinTap() / onFeedTap() / onPetTap() / onPlayTap()`
  - `MockHomeViewModel: HomeViewModel`：硬编码 mock 数据 + override 方法 print log
  - `RealHomeViewModel: HomeViewModel`：构造注入 AppState（`init(appState: AppState)`）+ override 方法接 UseCase；computed 派生 view-specific 状态
**And** "创建队伍" 按钮调 `state.onCreateTap()`
**And** "加入队伍" 按钮 trigger `state.showJoinModal = true`（HomeView 用 `.sheet(isPresented: $state.showJoinModal)` 挂 JoinRoomModal）
...
As an iPhone 用户,
I want Home Tab inRoom 态显示房间界面（房间号卡 + 共享舞台 + 成员列表 + 离开按钮）,
So that 我能与队友共处虚拟空间（视觉先实装，WS 同步留给 Epic 12）.

**Acceptance Criteria:**

**Given** Story 37.3 互斥状态机 + Story 37.6 primitives 已就绪
**When** 完成本 story
**Then** 在 `iphone/PetApp/Features/Room/Views/RoomScaffoldView.swift` 实装:
- 顶部：返回按钮 + 标题"队伍房间 / {猫名}的小屋"
- **房间号 Card**：直接显示 roomId 字符串（如「房间 1234567」），accessibility identifier `roomIdDisplay`；旁边复制按钮（点击复制 roomId 到 pasteboard，1.2s 内显示绿色对勾）
- 共享舞台 Card：粉橙渐变背景 + 云朵装饰 + 鱼/毛线球元素 + 4 只 MiniCat 上下弹跳（错峰 0.2s）
- 成员列表：4 格水平 HStack；mock 3 个成员 + 1 个虚线占位"+ 等待好友加入"；队长格"队长"小标签
- 底部"离开房间"PrimaryButton（次要按钮样式）
- 接缝：基类 `class RoomViewModel: ObservableObject` 含 `@Published var roomCodeForCopy: String / hostCatName / members / userIsHost`；abstract `func onLeaveTap() / onCopyTap()`；`MockRoomViewModel` / `RealRoomViewModel` 子类（Real 留 Epic 12.1 接 WS）
**And** **单元测试覆盖**（≥4 case）:
- happy: 注入 mock 4 成员 → View 渲染 4 格无占位
- happy: 注入 mock 2 成员 → View 渲染 2 实 + 2 虚线占位
- happy: 点击复制按钮 → onCopyTap 触发 + UI 显示绿色对勾 1.2s
- happy: 点击离开 → onLeaveTap 触发
**And** UITest: a11y identifier `roomIdDisplay` / `roomMember_0..3` / `copyButton` / `leaveButton` 可定位

### Story 37.9: WardrobeView Scaffold + WardrobeViewModel class 层次 + MockWardrobe

As an iPhone 用户,
I want Wardrobe Tab 显示 3D 试衣间式装扮浏览界面（预览区 + 分类 Tab + 3 列网格）,
So that 我能查看拥有的装扮和未解锁道具.

**Acceptance Criteria:**

**Given** Story 37.3 MainTabView + Story 37.6 primitives + RarityTag 已就绪
**When** 完成本 story
**Then** 在 `iphone/PetApp/Features/Wardrobe/Views/WardrobeView.swift` 实装:
- 顶部 Card：标题"{猫名} 的衣柜" + 收藏数（mock "36/53"）+ 钻石货币（mock "248"）+ "合成"小按钮（NavigationLink 入口预留，Story 33.1 实装真实 push）
- 预览区 Card：左 1/2 当前装扮猫预览；右 1/2 当前选中道具名 + RarityTag + 装备/卸下按钮
- 分类 Tab：横向 5 个（hat 🎩 / bow 🎀 / scarf 🧣 / outfit 👘 / bg 🏞️）
- 网格 LazyVGrid 3 列：每个道具方形 cell（emoji + 名字 + 稀有度色条）；未拥有半透明 + lock；已装备右上对勾
- mock Inventory（每分类 6-8 件，N/R/SR/SSR 各几个，部分 unowned）；维护本地 `equipped: Equipment` @State 用于切换展示
- 接缝：基类 `class WardrobeViewModel: ObservableObject` + Mock/Real 子类（Real 留 Epic 24.1）
**And** **单元测试覆盖**（≥4 case）:
...
**And** UITest: a11y identifier `wardrobeView` / `wardrobeCategory_hat..bg` / `wardrobeEquipButton` / `wardrobeComposeEntry` 可定位

### Story 37.10: FriendsView Scaffold + FriendsViewModel class 层次 + MockFriends

As an iPhone 用户,
I want Friends Tab 显示在线人数 + 我的房间提示条 + 在线/全部 Tab + 好友列表（含三态操作按钮）,
So that 我能查看好友状态、邀请好友、加入好友房间.

**Acceptance Criteria:**

**Given** Story 37.3 MainTabView + Story 37.6 primitives 已就绪
**When** 完成本 story
**Then** 在 `iphone/PetApp/Features/Friends/Views/FriendsView.swift` 实装:
- 顶部 Card：在线人数（mock "5 位好友在线"）+ 添加按钮（plus.circle.fill）
- 我的房间提示条 Card：仅当 `appState.currentRoomId != nil` 时显示；含房间 id 字符串 + "分享给好友"次要按钮（占位 toast，本 epic 不真分享）
- Tab：在线 / 全部 segmented control
- 好友列表 LazyVStack：FriendRow 含 Avatar（带在线小绿点）+ 名字 + "房间中"角标 + 状态文字 + 三态按钮（加入实心 / 邀请描边 / 离线灰字）
- 操作按钮（仅本地反馈）：
  - inRoom 好友"加入" → 直接调闭包（Real 时调 JoinRoomUseCase 传 friend.currentRoomId 字符串）
  - 在线好友"邀请" → 若 currentRoomId nil 触发本地 createTeam mock + toast；否则仅 toast
  - 离线 disabled
- mock 8 friend（三态各 2-3 个）；friends 数据归 ViewModel cache（不进 AppState，详见 ADR-0010 §3.2）
- 接缝：基类 `class FriendsViewModel: ObservableObject` + Mock/Real 子类
**And** **单元测试覆盖**（≥5 case）
**And** UITest: a11y identifier `friendsView` / `friendsTab_online/all` / `friendRow_<userId>` / `friendActionButton_<userId>` 可定位

### Story 37.11: ProfileView Scaffold + ProfileViewModel class 层次（含微信绑定卡 + Modal 视觉）

As an iPhone 用户,
I want Profile Tab 显示个人信息 / 收藏成就 / 设置入口 / 微信绑定卡 + Modal 视觉,
So that 我能查看资料、最近收藏、菜单入口（按钮均 toast 占位，真行为留给后续 epic）.

**Acceptance Criteria:**

**Given** Story 37.3 MainTabView + Story 37.6 primitives 已就绪 + PRD §4 已 PM 签字微信绑定 UI 视觉壳决议
**When** 完成本 story
**Then** 在 `iphone/PetApp/Features/Profile/Views/ProfileView.swift` 实装:
- 顶部渐变头图：theme.colors.accent → accent-soft 渐变 + Avatar + 用户名 + 用户 id + 称号 + "加入于"小药丸
- 统计卡（覆盖头图底部 1/3）：4 列（收藏品 / 好友 / 小猫等级 / 成就）
- 最近收藏：横向 5 个 Card（mock 5 件最近开箱）
- 微信绑定卡（按 ui_design/wechat_binding.md 视觉）：accent 渐变 + Icons.wechat + 标题"绑定微信，保护数据"+ Icons.shield 装饰；点击触发"绑定 Modal"显示
- 绑定 Modal：警告图标 Icons.warn + 高亮红字"一旦卸载本 App，您的小猫、收藏品、好友关系等所有数据都将被永久删除"+ "绑定微信"按钮（点击 toast "微信绑定（敬请期待）"，**不调真 OAuth**）
- 菜单列表 4 项：成就徽章 / 消息通知 / 喜欢的道具 / 设置（每项点击 toast）
- 接缝：基类 `class ProfileViewModel: ObservableObject` + Mock/Real 子类
**And** **单元测试覆盖**（≥3 case）
**And** UITest: a11y identifier `profileView` / `profileWeChatCard` / `profileWeChatModal` / `profileWeChatBindButton` / `profileMenu_<idx>` 可定位
**And** **PM 签字位**：本 story 落地前必须有 PM 在 Sprint Change Proposal v2.4 §8.2 签字位上签字（确认微信绑定 UI 视觉壳不突破 PRD §4 「不实现 UI」边界）

### Story 37.12: JoinRoomModal + 跨屏 join 链路（roomId 字符串 + 直白 UI）

As an iPhone 用户,
I want JoinRoomModal 接受房间号字符串输入，跨屏 join 链路统一走 roomId 字符串,
So that A 分享给 B 的房间号 B 能直接输入加入，无 sender/receiver 闭环问题.

**Acceptance Criteria:**

**Given** Story 37.4 AppState + Story 37.7 HomeView TeamIdleCard "加入队伍"按钮 + Story 37.10 FriendsView FriendRow "加入"按钮
**When** 完成本 story
**Then** 在 `iphone/PetApp/Shared/Modals/JoinRoomModal.swift` 实装:
- 底部 sheet（`.sheet(isPresented: $homeViewModel.showJoinModal)` 挂在 HomeView 内）：背景遮罩 0.45 + 卡片从下方 20pt 上滑 0.3s
- 卡片 Card 圆角 26 + theme.colors.surface + 标题"加入队伍" + 关闭按钮（Icons.close）
- 大输入框：Icons.paw prefix + 等宽字体 + 自动 trim + 限 64 字符
- 格式提示"输入好友分享给你的房间号"灰字 small（**不**暗示纯数字）
- 取消 / 确定加入 两按钮：仅输入 trim 后非空启用确定（不做客户端格式校验，server 决定合法性）
- 确定加入 → 调用通过构造注入的 `onConfirm: (String) async -> Void` 闭包；**不直接调 AppState 或 UseCase**（modal 与业务解耦）
- HomeScreen 触发：HomeView 内 `.sheet(isPresented: $state.showJoinModal)` 挂；showJoinModal 唯一 owner = HomeViewModel 基类的 `@Published var showJoinModal: Bool`
- FriendsScreen "加入"按钮（好友在房间中）触发：解析 `friend.currentRoomId: String` **直接**调 JoinRoomUseCase（不弹 Modal）
**And** **单元测试覆盖**（≥5 case，纯 XCTest）:
- happy: 输入 "1234567" → 确定按钮启用
- happy: 输入 "1234567" 点确定 → onConfirm 闭包被调用 + 参数 == "1234567"
- edge: 空输入 → 确定按钮 disabled
- edge: 仅空格输入 → trim 后判定空 → disabled
- happy: 输入超过 64 字符 → 截断在 64 字符
**And** UITest: HomeView 点 "加入队伍" → JoinRoomModal a11y identifier `joinRoomModal` 出现 → 输入 "1234567" → 点 confirm → modal dismiss + RoomView 出现
...
So that UI 测试有可依赖的定位锚点 + 严守 ADR-0002 §3.1 的合规兜底（非 snapshot 等价）.

**Acceptance Criteria:**

**Given** Story 37.5-37.12 全部完成
**When** 完成本 story
**Then** 扩展 `iphone/PetApp/Shared/Constants/AccessibilityID.swift` 覆盖：
- 5 屏的 Tab Bar 定位（tab_home / tab_wardrobe / tab_friends / tab_profile）
- HomeView 全部交互元素
- RoomView 全部（returnButton / `roomIdDisplay` / copyButton / 4 个成员位 / 离开按钮）—— **a11y 命名严格 `roomIdDisplay`，不允许旧名 `roomCodeDisplay`**
- WardrobeView / FriendsView / ProfileView 全部
- JoinRoomModal 全部（input field / cancel / confirm）
**And** 静态校验脚本 `iphone/scripts/check_a11y_coverage.sh`：grep Features/ + Shared/Modals 内 SwiftUI View body 含 button/list 等交互元素的 accessibilityIdentifier 注解；不达标列出并 fail
**And** 静态校验脚本 `iphone/scripts/check_no_apiclient_in_features.sh`：grep Features/{Home,Room,Wardrobe,Friends,Profile} + Shared/Modals + Core/DesignSystem 内 import APIClient / Repository / UseCase（除显式 RealViewModel wire 模式）；违规列出 fail
**And** 视觉回归 review checklist 文档 `iphone/docs/visual-review-checklist.md`：每屏 6-10 个 manual visual check + 截图位 + 用于 PR review 时手动逐项打勾（**注**：合规兜底，不声称等价 snapshot 测试覆盖度）
...
As an PM / SM,
I want 一份白名单文档明确列出 iphone/ui_design 内本期 Epic 37 不做的元素，且每条带 PRD 边界依据,
So that 后续 sprint 不重复争论"为什么 X 没做" + 透明传达取舍.
...
- `tweaks-panel.jsx`（开发时主题切换调试面板）→ 不做 → 永不实装
- 三主题切换 UI → 不做（仅 Theme stub）→ 后续 mini-epic
- `wechat_binding.md` 真实 OAuth → 不做（视觉壳做，按钮 toast）→ 节点 12 后另起 epic
- Profile 顶部 bell（消息通知）真实通知中心 → 不做 → 后续 epic
- Profile "成就徽章"页 / "喜欢的道具"页详情 → 不做 → 后续 epic
- HomeView 互动后状态条变化（喂食后饱食 +5 等）→ 不做 → 节点 3 起接 LoadHomeUseCase 后端真实状态
- WardrobeView 钻石货币真实数值更新 → 不做 → 后续 epic（与商城联动；本 MVP 不含商城）
- 小猫 3D 模型（USDZ / RealityKit）→ 不做（用 SF Symbol cat.fill 占位）→ 美术资源就位后另起 spike
- 装扮道具 emoji 占位 → 仅本期 Scaffold 用（节点 10 起 Story 30.x 替换 SpriteRenderer，仅猫身上；仓库 grid emoji 保留）
- K3M9P2 等美化别名 → 不做（UI 全用 roomId 字符串，避免 sender/receiver UX 闭环问题；未来若产品需要可在专门 spike 设计可逆契约）
